...
import { Children, ComponentProps, isValidElement, ChangeEvent, CSSProperties, DragEvent as ReactDragEvent, FormEvent, PointerEvent as ReactPointerEvent, ReactNode, useCallback, useDeferredValue, useEffect, useMemo, useRef, useState } from "react";
import { useSemsAuth, WORKSPACE_CHANGE_EVENT } from "@/components/auth-context";
import { SEMS_ROLE_LABELS } from "@/lib/access-control";
import "./operations.css";
import OperationsAudit from "@/components/operations-audit";
import { OperationsSiteCoverage } from "@/components/operations-site-coverage";
import { OperationsWorkbench, DataStatusFields } from "@/components/operations-workbench";
import type { OperationCheck, QualityResolution, ReviewHistoryEntry } from "@/components/operations-workbench";
import { csvEscape } from "@/lib/csv-export";
import { OperationsQuality } from "@/components/operations-quality";
import { DATA_STATUS_LABELS, dataValueStatus, validateDataValue, aggregateMetric } from "@/lib/metric-aggregation";
import type { DataStatus } from "@/lib/metric-aggregation";
import { calculateEmission, isFactorEffective } from "@/lib/factor-calculation";
import { getWorkspaceClosingIssues } from "@/lib/workspace-closing";
import type { FactorSnapshot } from "@/lib/factor-calculation";
import { IMPORT_FIELDS, suggestColumnMapping, validateActivityImport, validateImportMapping } from "@/lib/activity-import";
import type { ImportColumnMapping } from "@/lib/activity-import";
import {
...
type SettingTab = "organization" | "factors" | "criteria" | "notifications" | "data";
...
type OperationalReviewFields = {
  dataStatus?: DataStatus;
  reviewNote?: string;
  reviewedAt?: string;
  reviewedBy?: string;
  reviewHistory?: ReviewHistoryEntry[];
  qualityResolutions?: Record<string, QualityResolution>;
};
type ActivityRecord = OperationalReviewFields & {
  factorId?: string;
...
type Indicator = {
  ...
  organizationAggregation?: "합계" | "평균" | "비율";
...
type MetricSubmission = OperationalReviewFields & {
  active?: boolean;
  numerator?: number;
  denominator?: number;
  id: number;
...
const navItems: { id: View; label: string; icon: IconName }[] = [
  ...
  { id: "quality", label: "입력 오류·확인 사항", icon: "alert" },
  ...
  { id: "indicators", label: "ESG 지표 기준", icon: "list" },
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`status-badge ${key}`}><span className="status-dot" />{({검토대기:"제출 완료·검토 중",확정:"승인 완료",작성중:"임시저장",반려:"수정 요청"} as Record<string,string>)[status]??status}</span>;
}
...
function Overlay({ title, eyebrow, description, onClose, children, size = "normal" }: { title: string; eyebrow: string; description?: string; onClose: () => void; children: ReactNode; size?: "normal" | "small" | "wide" }) {
  ...
  useEffect(() => { const close = (event:KeyboardEvent) => { if(event.key!=="Escape")return;const layers=document.querySelectorAll(".modal-backdrop");if(layers[layers.length-1]===backdrop.current){event.preventDefault();onClose();} };window.addEventListener("keydown",close);document.body.classList.add("menu-open");return()=>{window.removeEventListener("keydown",close);if(document.querySelectorAll(".modal-backdrop").length<=1)document.body.classList.remove("menu-open");}; },[onClose]);
  return <div ref={backdrop} className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}><div className={`record-modal ${size === "small" ? "small-modal" : size === "wide" ? "wide-modal" : ""}`} role="dialog" aria-modal="true" aria-label={title}><div className="modal-header"><div><span>{eyebrow}</span><h2>{title}</h2>{description && <p>{description}</p>}</div><button className="icon-button" onClick={onClose} aria-label="닫기"><Icon name="close" /></button></div>{children}</div></div>;
}
...
export default function Home() {
  ...
  const [storedMetricRequests, setMetricRequests] = useState<MetricRequest[]>([]);
  const [storedMetricSubmissions, setStoredMetricSubmissions] = useState<MetricSubmission[]>([]);
  const metricSubmissions = useMemo(() => storedMetricSubmissions.filter(row => row.active !== false), [storedMetricSubmissions]);
  const setMetricSubmissions = useCallback((value: MetricSubmission[] | ((current: MetricSubmission[]) => MetricSubmission[])) => {
    setStoredMetricSubmissions(current => {
      const rows = typeof value === "function" ? value(current.filter(row => row.active !== false)) : value;
      return [...current.filter(row => row.active === false && !rows.some(next => next.id === row.id)), ...rows];
    });
  }, [setStoredMetricSubmissions]);
  ...
  const [storedPeriods, setPeriods] = useState<CollectionPeriod[]>([]);
  const [deadlineClock, setDeadlineClock] = useState(() => Date.now());
  useEffect(() => {
    const refresh = () => setDeadlineClock(previous => {
      const now = Date.now();
      const koreanDay = (time: number) => Math.floor((time + 9 * 60 * 60 * 1000) / 86400000);
      return koreanDay(previous) === koreanDay(now) ? previous : now;
    });
    const timer = window.setInterval(refresh, 1000);
    window.addEventListener("focus", refresh);
    return () => { window.clearInterval(timer); window.removeEventListener("focus", refresh); };
  }, []);
  const periods = useMemo(() => storedPeriods.map(period => effectiveCollection(period, deadlineClock)), [storedPeriods, deadlineClock]);
  const metricRequests = useMemo(() => storedMetricRequests.map(request => effectiveCollection(request, deadlineClock)), [storedMetricRequests, deadlineClock]);
  ...
  const [coveragePreview,setCoveragePreview]=useState<{title:string;lines:string[]}|null>(null);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        const savedMetricSubmissions = localStorage.getItem("sems2-metric-submissions"); if (savedMetricSubmissions) setStoredMetricSubmissions(normalizeMetricSubmissions(JSON.parse(savedMetricSubmissions) as MetricSubmission[],indicatorIdMap,normalizedIndicators));
  ...
  useEffect(() => {
    ...
    const payload: Record<string, unknown> = {
      ...
      "sems2-metric-requests": storedMetricRequests,
      "sems2-metric-submissions": storedMetricSubmissions,
  ...
  }, [storedPeriods, records, factors, formulas, activityMasters, assetUnits, scope3Fields, disclosureStandards, regulations, suppliers, productMaterials, transportRoutes, disclosureMappings, scope3Requests, diagnosticTemplates, supplyChainAssessments, evidence, indicators, storedMetricRequests, storedMetricSubmissions, reports, targets, plans, audit, criteria, noticePrefs, organizations, hydrated, canWrite]);
  ...
  const navigate = (view: View) => {
    if(view==="collection")setCollectionKind("ghg");
  ...
  const saveRecord = (record: ActivityRecord) => {
    if (!canCollect(periods.find(item => item.id === record.collectionId))) { showToast("제출이 마감되었습니다. 관리자에게 제출기한 연장을 요청해 주세요."); return; }
    ...
    if (duplicate) { showToast("같은 수집기간·사업장·자료 기준월·활동자료가 이미 등록되어 있습니다."); return; }
    ...
    setModalOpen(false); setEditing(null); showToast("서버에 저장 중입니다. 상단 저장 상태를 확인해 주세요.");
  };
  const saveBatchRecords = (rows: ActivityRecord[]) => {
    if (rows.some(row => !canCollect(periods.find(item => item.id === row.collectionId)))) { showToast("제출이 마감되었습니다. 관리자에게 제출기한 연장을 요청해 주세요."); return; }
    if (!canWrite) { showToast("조회자는 자료를 저장할 수 없습니다."); return; }
    ...
    setBatchOpen(false);
    showToast("서버에 저장 중입니다. 상단 저장 상태를 확인해 주세요.");
  };
  ...
  const exportBackup = () => {
    const payload = { version: 8, exportedAt: new Date().toISOString(), periods, records, factors, formulas, activityMasters, assetUnits, scope3Fields, disclosureStandards, regulations, suppliers, productMaterials, transportRoutes, disclosureMappings, scope3Requests, diagnosticTemplates, supplyChainAssessments, evidence, indicators, metricRequests, metricSubmissions: storedMetricSubmissions, reports, targets, plans, audit, criteria, noticePrefs, organizations };
  ...
  const restoreBackup = (payload: Record<string, unknown>) => {
    ...
    if (Array.isArray(payload.metricRequests)) setMetricRequests(normalizeMetricRequests(payload.metricRequests as MetricRequest[],restoredIndicatorIdMap)); if (Array.isArray(payload.metricSubmissions)) setStoredMetricSubmissions(normalizeMetricSubmissions(payload.metricSubmissions as MetricSubmission[],restoredIndicatorIdMap,restoredIndicators));
  ...
  const selectCoverage=(item:CoverageItem<string|number>)=>{
    if(!item.requestIds.length){setCollectionRequestView("requests");showToast("이 항목을 포함하는 수집 요청을 개설해 주세요.");return;}
    if(collectionKind==="ghg"){
      const existing=records.find(row=>item.requestIds.includes(row.collectionId??"")&&row.active!==false&&row.company===item.company&&row.site===item.site&&row.period===item.month&&row.scope===item.targetId);
      const period=periods.find(row=>item.requestIds.includes(row.id));
      if(existing&&(existing.locked||["확정","검토대기"].includes(existing.status)||!canCollect(period))){setCoveragePreview({title:existing.source,lines:[existing.company+" · "+existing.site+" · "+existing.period,existing.status+" / "+DATA_STATUS_LABELS[dataValueStatus({value:existing.usage,dataStatus:existing.dataStatus})],formatNumber(existing.usage,3)+" "+existing.unit+" → "+formatNumber(existing.emissions,3)+" tCO₂e","담당자: "+existing.owner,"입력 설명: "+(existing.description||"없음"),"검토 의견: "+(existing.reviewNote||"없음")]});return;}
      if(existing){openForm(existing);return;}if(!canCollect(period)){showToast("현재 입력 가능한 수집기간이 아닙니다.");return;}
      const factor=factors.find(row=>row.scope===item.targetId&&isFactorEffective(row,item.month));if(!period||!factor){showToast("현재 기간에 사용할 배출계수를 먼저 등록해 주세요.");return;}
      openForm({id:0,collectionId:period.id,company:item.company,site:item.site??organizations[item.company]?.[0]??"",period:item.month,scope:factor.scope,category:factor.category,source:factor.source,factorId:factor.id,usage:0,dataStatus:"missing",unit:factor.activityUnit,factor:factor.value,emissions:0,owner:profile.display_name||profile.email||"",department:profile.department||"",status:"작성중",evidence:"",active:true,updatedAt:nowLabel()});return;
    }
    const request=metricRequests.find(row=>item.requestIds.includes(row.id));const indicator=indicators.find(row=>row.id===item.targetId);if(!request||!indicator)return;
    const submission=metricSubmissions.find(row=>item.requestIds.includes(row.requestId)&&row.company===item.company&&row.site===item.site&&row.period===item.month&&row.indicatorId===item.targetId);
    if(!canCollect(request)||submission&&["확정","검토대기"].includes(submission.status)){setCoveragePreview({title:indicator.name,lines:[item.company+" · "+(item.site??"")+" · "+item.month,"처리상태: "+(submission?.status??"미입력"),submission?formatNumber(submission.value,3)+" "+submission.unit:"제출값 없음","담당자: "+(submission?.owner||indicator.owner||"미지정"),"입력 설명: "+(submission?.description||"없음"),"검토 의견: "+(submission?.reviewNote||"없음")]});return;}
    setQuickMetric({request,indicator,company:item.company,site:item.site,period:item.month,submission});
  };

  const roleLabel = SEMS_ROLE_LABELS[profile.role];
  const syncLabel = syncStatus === "conflict" ? "다른 변경사항 확인 필요" : syncStatus === "saving" ? "서버 저장 중" : syncStatus === "error" ? "저장 실패 · 새로고침하지 마세요" : canWrite ? "서버 저장 완료" : "서버 조회 전용";
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      ...
      <main className="content">
        ...
        {activeView === "targets" && <TargetsAndPlans targets={targets} plans={plans} records={records} organizations={organizations} canManage={canManage} onTargetsChange={items=>{if(!canManage){showToast("감축목표 관리는 관리자 권한이 필요합니다.");return;}setTargets(items);}} onPlansChange={items=>{if(!canManage){showToast("감축계획 관리는 관리자 권한이 필요합니다.");return;}setPlans(items);}} addAudit={addAudit} showToast={showToast} />}
        ...
        {activeView === "audit" && <>{canManage&&<OperationsAudit/>}<details><summary className="operations-section-title">기존 업무 이벤트 기록 (최근 500건)</summary><AuditLog items={audit} showToast={showToast}/></details></>}
        {activeView === "settings" && <Settings factors={factors} records={records} onFactorsChange={setFactors} criteria={criteria} onCriteriaChange={setCriteria} noticePrefs={noticePrefs} onNoticePrefsChange={setNoticePrefs} organizations={organizations} onOrganizationsChange={setOrganizations} onExport={exportBackup} onRestore={restoreBackup} showToast={showToast} />}
      </main>
    </div>
    {coveragePreview&&<Overlay title={coveragePreview.title} eyebrow="COLLECTION DATA" description="선택한 사업장·기준월의 자료와 검토 상태입니다." onClose={()=>setCoveragePreview(null)}><div className="form-section operations-description">{coveragePreview.lines.map((line,index)=><p key={index}>{line}</p>)}</div><div className="modal-footer"><button className="primary-button" onClick={()=>setCoveragePreview(null)}>확인</button></div></Overlay>}
    {quickMetric&&<MetricSubmissionForm context={quickMetric} organizations={organizations} defaultOwner={profile.display_name||profile.email||""} defaultDepartment={profile.department||""} canManage={canManage} onClose={()=>setQuickMetric(null)} onSave={submission=>{if(!canCollect(metricRequests.find(item=>item.id===submission.requestId))){showToast("제출이 마감되었습니다. 관리자에게 제출기한 연장을 요청해 주세요.");return;}const saved={...submission,id:submission.id||Date.now(),updatedAt:nowLabel()};setMetricSubmissions(current=>current.some(row=>row.id===saved.id)?current.map(row=>row.id===saved.id?saved:row):[saved,...current]);addAudit(saved.status==="검토대기"?"정량데이터 제출":"정량데이터 임시저장",saved.company,`${saved.period} · ${quickMetric.indicator.name}`);setQuickMetric(null);showToast("서버에 저장 중입니다. 상단 저장 상태를 확인해 주세요.");}}/>}
...
function CollectionCoverage({
  ...
  onSelect:(item:CoverageItem<string|number>)=>void;
}) {
  ...
  const coverageItems: CoverageItem<string | number>[] = useMemo(() => kind === "ghg"
    ? buildGHGCoverage({
      ...
      requests: periods.map(period => ({
        ...
        sitesByCompany:period.sitesByCompany,
      })),
      records: records.map(record => ({
        requestId: record.collectionId,
        company: record.company,
        site:record.site,
        dataStatus:dataValueStatus({value:record.usage,dataStatus:record.dataStatus}),
        month: record.period,
    ...
    : buildMetricCoverage({
      ...
      requests: metricRequests.map(request => ({
        ...
        sitesByCompany:request.sitesByCompany,
      })),
      submissions: metricSubmissions.map(submission => ({
        requestId: submission.requestId,
        company: submission.company,
        site:submission.site,
        dataStatus:submission.detailRows?.length&&submission.dataStatus==="actual"?"actual":dataValueStatus({value:submission.value,dataStatus:submission.dataStatus}),
        month: submission.period,
        targetId: submission.indicatorId,
        status: submission.status,
      })),
      today,
    }), [companies, kind, metricRequests, metricSubmissions, periods, records, targetOptions, today, year, organizations]);
  ...
  return <><PageHeader eyebrow="COLLECTION COVERAGE" title="사업장별 요청·제출 현황" description="연도·월·법인·수집 항목별로 요청 누락과 입력·검토·확정 상태를 확인합니다."/>
    <section className="coverage-summary" aria-label="요청·제출 현황 상태 요약">
    ...
    <section className="card coverage-workspace">
      ...
      <OperationsSiteCoverage kind={kind} year={year} sitesByCompany={organizations} items={filteredItems} targetLabel={targetLabel} onSelect={onSelect}/>
...
function NotificationPanel({ periods, records, targets, plans, onClose, onRead }: { periods: CollectionPeriod[]; records: ActivityRecord[]; targets: ReductionTarget[]; plans: ReductionPlan[]; onClose: () => void; onRead: () => void }) {
  ...
  return <div className="notification-panel"><div className="panel-title"><strong>업무 알림</strong><button onClick={onClose} aria-label="알림 닫기"><Icon name="close" size={16} /></button></div>
    ...
    <div className="notification-item"><span className="notification-icon success"><Icon name="check" size={17} /></span><div><strong>검토 대기 {pending}건</strong><p>관리자 검토와 확정 처리가 필요합니다.</p><small>현재 기준</small></div></div>
...
function ProfilePanel({profileName,detail,canManage,onSettings,onSignOut}:{profileName:string;detail:string;canManage:boolean;onSettings:()=>void;onSignOut:()=>Promise<void>}){return <div className="profile-panel"><div><strong>{profileName}</strong><span>{detail}</span></div>{canManage&&<button onClick={onSettings}><Icon name="settings" size={16}/>시스템 설정</button>}<button className="profile-logout" onClick={()=>void onSignOut()}><Icon name="logout" size={16}/>로그아웃</button><p>운영 데이터는 Supabase 서버에 자동 저장되며 계정 권한에 따라 조회·수정 범위가 제한됩니다.</p></div>}
...
function Dashboard({ records, periods, metricRequests, metricSubmissions, indicators, targets, plans, organizationNames, currentOrganization, canWrite, canReview, onNavigate, onOpenInputRequest, onNew }: { records: ActivityRecord[]; periods: CollectionPeriod[]; metricRequests: MetricRequest[]; metricSubmissions: MetricSubmission[]; indicators: Indicator[]; targets: ReductionTarget[]; plans: ReductionPlan[]; organizationNames: string[]; currentOrganization: string; canWrite: boolean; canReview: boolean; onNavigate: (view: View) => void; onOpenInputRequest: (kind: "ghg"|"esg") => void; onNew: () => void }) {
  ...
  const isDataInputter = canWrite && !canReview;
  const inputRequests = isDataInputter ? [
    ...periods.filter(period=>period.status==="수집중").flatMap(period=>{
      const tasks=buildGHGCollectionTasks(period).filter(task=>!currentOrganization||task.company===currentOrganization);
      const remaining=tasks.filter(task=>!records.some(record=>record.collectionId===period.id&&record.company===task.company&&record.scope===task.targetId&&record.period===task.period&&["검토대기","확정"].includes(record.status))).length;
      return tasks.length&&remaining?[{id:`ghg-${period.id}`,kind:"ghg" as const,title:period.name,detail:`온실가스 · ${remaining}/${tasks.length}개 항목 입력 필요`,dueDate:period.dueDate}]:[];
    }),
    ...metricRequests.filter(request=>request.status==="수집중").flatMap(request=>{
      const tasks=buildMetricCollectionTasks(request,indicators).filter(task=>!currentOrganization||task.company===currentOrganization);
      const remaining=tasks.filter(task=>!metricSubmissions.some(submission=>submission.requestId===request.id&&submission.company===task.company&&submission.indicatorId===task.targetId&&submission.period===task.period&&["검토대기","확정"].includes(submission.status))).length;
      return tasks.length&&remaining?[{id:`esg-${request.id}`,kind:"esg" as const,title:request.title,detail:`기타 ESG · ${remaining}/${tasks.length}개 항목 입력 필요`,dueDate:request.dueDate}]:[];
    }),
  ].sort((a,b)=>a.dueDate.localeCompare(b.dueDate)):[];
  return <><PageHeader eyebrow={`${year} ESG PERFORMANCE`} title="ESG 통합 대시보드" description="현재 계정에 허용된 법인의 ESG 데이터 수집 현황과 주요 성과를 한눈에 확인합니다."><label className="year-select"><Icon name="calendar" size={17} /><select value={year} onChange={e => setRequestedYear(e.target.value)}>{years.map(y => <option key={y}>{y}</option>)}</select></label>{canWrite&&<button className="primary-button" onClick={onNew}><Icon name="plus" size={17} />자료 입력</button>}</PageHeader>
    {activePeriod ? <section className="notice-banner"><div className="notice-icon"><Icon name="alert" /></div><div><strong>{activePeriod.name} · {activePeriod.status === "수집중" ? `제출 마감까지 ${Math.max(0, daysUntil(activePeriod.dueDate))}일` : "관리자 검토 진행 중"}</strong><p>{activePeriod.dataFrom}~{activePeriod.dataTo} 귀속자료 · 검토 대기 {pending.length}건</p></div><button onClick={() => onNavigate(activePeriod.status === "수집중" || !canReview ? "collection" : "review")}>{activePeriod.status === "수집중" || !canReview ? "수집 현황 보기" : "검토 화면 열기"} <Icon name="arrow" size={16} /></button></section> : <section className="notice-banner neutral"><div className="notice-icon"><Icon name="calendar" /></div><div><strong>현재 진행 중인 수집기간이 없습니다.</strong><p>{canReview?"정량데이터 수집 메뉴에서 다음 정기수집을 개설해 주세요.":"새 수집 요청이 등록되면 이 화면에 표시됩니다."}</p></div>{canReview&&<button onClick={() => onNavigate("collection-request")}>기간 설정 <Icon name="arrow" size={16} /></button>}</section>}
    ...
    <section className="dashboard-grid"><article className="card emissions-chart-card"><CardHeader title="월별 온실가스 배출 추이" subtitle="Scope 1·2·3 합산 배출량" action="단위: tCO₂e"/><div className="chart-legend"><span><i className="scope1"/>Scope 1</span><span><i className="scope2"/>Scope 2</span><span><i className="scope3"/>Scope 3</span></div><div className="bar-chart"><div className="axis-labels"><span>{formatNumber(chartMax,0)}</span><span>{formatNumber(chartMax*.75,0)}</span><span>{formatNumber(chartMax*.5,0)}</span><span>{formatNumber(chartMax*.25,0)}</span><span>0</span></div><div className="grid-lines"><i/><i/><i/><i/><i/></div><div className="bars">{monthly.map(item => { const sum=item.s1+item.s2+item.s3; return <div className="bar-group" key={item.month}><div className="bar-stack chart-scaled" style={{height:`${Math.max(sum/chartMax*170,4)}px`}}><span className="scope3" style={{height:`${sum ? item.s3/sum*100 : 0}%`}}/><span className="scope2" style={{height:`${sum ? item.s2/sum*100 : 0}%`}}/><span className="scope1" style={{height:`${sum ? item.s1/sum*100 : 0}%`}}/>{sum>0&&<b>{formatNumber(sum,0)}</b>}</div><small>{item.month}</small></div>; })}</div></div></article>
      ...
      <article className="card review-card">{isDataInputter?<><CardHeader title="입력 요청 자료" subtitle={`${inputRequests.length}건의 수집 요청이 입력을 기다리고 있습니다.`} action={<button onClick={()=>onOpenInputRequest(inputRequests[0]?.kind??"ghg")}>전체보기 <Icon name="arrow" size={14}/></button>}/><div className="review-list">{inputRequests.length?inputRequests.slice(0,3).map(request=><button className="review-item" key={request.id} onClick={()=>onOpenInputRequest(request.kind)}><div className={`source-icon ${request.kind==="ghg"?"green":"blue"}`}><Icon name={request.kind==="ghg"?"leaf":"list"} size={18}/></div><div><strong>{request.title}</strong><span>{request.detail}</span></div><em>{request.dueDate.slice(5).replace("-",".")} 마감</em><Icon name="chevron" size={16}/></button>):<div className="empty-state compact"><Icon name="check"/><strong>현재 입력할 요청 자료가 없습니다.</strong></div>}</div></>:<><CardHeader title={canReview?"검토 대기 자료":"제출한 검토 대기 자료"} subtitle={`${pending.length}건의 자료가 확인을 기다리고 있습니다.`} action={canReview?<button onClick={()=>onNavigate("review")}>전체보기 <Icon name="arrow" size={14}/></button>:undefined}/><div className="review-list">{pending.length ? pending.slice(0,3).map(record=><button className="review-item" key={record.id} onClick={()=>onNavigate(canReview?"review":"collection")}><div className={`source-icon ${record.scope==="Scope 1"?"green":"blue"}`}><Icon name={record.scope==="Scope 1"?"droplet":"bolt"} size={18}/></div><div><strong>{record.category} · {record.source}</strong><span>{record.company} / {record.site}</span></div><em>{formatNumber(record.emissions,1)} t</em><Icon name="chevron" size={16}/></button>):<div className="empty-state compact"><Icon name="check"/><strong>검토 대기 자료가 없습니다.</strong></div>}</div></>}</article></section></>;
}
...
function Periods({ periods, records, organizationNames, organizations, onChange, addAudit, showToast }: { periods: CollectionPeriod[]; records: ActivityRecord[]; organizationNames: string[]; organizations:Record<string,string[]>; onChange: (items: CollectionPeriod[]) => void; addAudit: (action: string, target: string, detail: string, actor?: string) => void; showToast: (m: string) => void }) {
  ...
  const updateStatus = (period: CollectionPeriod, status: PeriodStatus) => {
    if (status === "수집중" && submissionExpired(period)) { showToast("설정 수정에서 제출 마감일을 먼저 연장해 주세요."); return; }
    if(["마감","잠금"].includes(status)){const issues=getWorkspaceClosingIssues({kind:"ghg",request:period,rows:records,organizations});if(issues.length){setClosingIssues(issues.map(issue=>issue.message));return;}}
    ...
    showToast("기간 변경을 서버에 저장 중입니다. 상단 저장 완료 후 자료를 수정해 주세요.");
  };
  const save = (period: CollectionPeriod) => {
    ...
    if(["마감","잠금"].includes(period.status)&&periods.find(item=>item.id===period.id)?.status!==period.status){const issues=getWorkspaceClosingIssues({kind:"ghg",request:period,rows:records,organizations});if(issues.length){setClosingIssues(issues.map(issue=>issue.message));return;}}
    const saved={...period,taskKeys,sitesByCompany:period.sitesByCompany??Object.fromEntries(period.companies.map(company=>[company,organizations[company]??[]]))};
  ...
  return <><PageHeader eyebrow="COLLECTION PERIOD" title="수집 기간 관리" description="귀속기간, 대상 법인·Scope, 제출·검토 마감과 잠금 상태를 운영합니다.">{canManage&&<button className="primary-button" onClick={() => setEditing("new")}><Icon name="plus" size={17}/>수집기간 개설</button>}</PageHeader>
    ...
    <section className="period-grid">{periods.map(period => {
      const rows = records.filter(record => record.collectionId === period.id&&record.active!==false);
      const tasks=tasksAtSites(buildGHGCollectionTasks(period),organizations,period.sitesByCompany);
      const submitted = new Set(rows.filter(record => ["검토대기","확정"].includes(record.status)).map(record=>collectionTaskKey(record.company,record.scope,record.period,record.site))).size;
      const confirmed = new Set(rows.filter(record => record.status === "확정").map(record=>collectionTaskKey(record.company,record.scope,record.period,record.site))).size;
      const completion = tasks.length ? Math.round(confirmed / tasks.length * 100) : 0;
      return <article className="card period-card" key={period.id}><div className="period-card-top"><div><span className={`status-badge ${periodTone(period.status)}`}><span className="status-dot"/>{period.status==="검토중"?"제출 마감 · 검토중":period.status}</span><h2>{period.name}</h2><p>{period.description}</p></div></div>
        <div className="period-dates"><div><span>귀속기간</span><strong>{period.dataFrom === period.dataTo ? period.dataFrom : `${period.dataFrom} ~ ${period.dataTo}`}</strong></div><div><span>수집기간</span><strong style={koreanHolidayName(period.dueDate)?{color:"#d32f2f"}:undefined}>{period.openDate} ~ {period.dueDate}{koreanHolidayName(period.dueDate)&&<em style={{display:"block",marginTop:4,fontStyle:"normal",fontSize:12,fontWeight:700,color:"#d32f2f"}}>{koreanHolidayName(period.dueDate)}</em>}</strong></div><div><span>검토마감</span><strong style={koreanHolidayName(period.reviewDate)?{color:"#d32f2f"}:undefined}>{period.reviewDate}{koreanHolidayName(period.reviewDate)&&<em style={{display:"block",marginTop:4,fontStyle:"normal",fontSize:12,fontWeight:700,color:"#d32f2f"}}>{koreanHolidayName(period.reviewDate)}</em>}</strong></div></div>
        ...
        {canManage&&<div className="period-actions">{period.status === "예정" && <button className="primary-button compact" onClick={()=>updateStatus(period,"수집중")}>수집 시작</button>}{period.status === "수집중" && <button className="primary-button compact" onClick={()=>updateStatus(period,"검토중")}>제출 마감·검토 시작</button>}{period.status === "검토중" && <button className="primary-button compact" onClick={()=>updateStatus(period,"마감")}>검토 완료·마감</button>}{period.status === "마감" && <button className="primary-button compact" onClick={()=>updateStatus(period,"잠금")}>확정자료 잠금</button>}{["검토중","마감","잠금"].includes(period.status) && <button className="secondary-button compact" onClick={()=>updateStatus(period,"수집중")}>기간 다시 열기</button>}<button className="secondary-button compact" onClick={()=>setEditing(period)}><Icon name="edit" size={14}/>설정 수정</button><button className="danger-button compact" onClick={()=>remove(period)}><Icon name="trash" size={14}/>삭제</button></div>}
      </article>;
    })}</section>
    ...
    {closingIssues.length>0&&<ClosingIssues messages={closingIssues} onClose={()=>setClosingIssues([])}/>}
  </>;
}

function ClosingIssues({messages,onClose}:{messages:string[];onClose:()=>void}){
  return <Overlay title="마감 전 확인할 자료" eyebrow="PERIOD CLOSING" description={"미입력·미확정·검증 오류 "+messages.length+"건을 확인해 주세요. 자료가 모두 확정된 뒤 마감할 수 있습니다."} onClose={onClose}><div className="form-section operations-closing"><ol>{messages.map((message,index)=><li key={index}>{message}</li>)}</ol></div><div className="modal-footer"><button type="button" className="primary-button" onClick={onClose}>확인</button></div></Overlay>;
}
...
function Collection({ records, periods, criteria, organizationNames, onNew, onBulk, onEdit, onChange, showToast }: { records: ActivityRecord[]; periods: CollectionPeriod[]; criteria: CollectionCriteria; organizationNames: string[]; onNew: () => void; onBulk: () => void; onEdit: (record: ActivityRecord) => void; onChange: (records: ActivityRecord[], auditInfo?: { action: string; target: string; detail: string }) => void; showToast: (m: string) => void }) {
  ...
  const baseFiltered=records.filter(r=>r.active!==false&&(periodId==="전체"||r.collectionId===periodId)&&(company==="전체 법인"||r.company===company)&&(scope==="전체 Scope"||r.scope===scope)&&`${r.company} ${r.site} ${r.category} ${r.source} ${r.owner} ${r.description??""}`.toLowerCase().includes(search.toLowerCase()));
  ...
  const exportCsv=()=>{downloadCsv("SEMS_activity_data.csv",["수집기간","법인","사업장","자료 기준월","Scope","구분","배출원","사용량","단위","배출계수","배출량(tCO2e)","증빙","설명","담당자","부서","상태"],filtered.map(r=>[periods.find(p=>p.id===r.collectionId)?.name??"",r.company,r.site,r.period,r.scope,r.category,r.source,r.usage,r.unit,r.factor,r.emissions,r.evidence,r.description??"",r.owner,r.department,r.status]));showToast("현재 조회 결과를 내려받았습니다.");};

  const withdrawRecord=(record:ActivityRecord)=>{if(record.status!=="검토대기"){showToast("검토 대기 중인 자료만 회수할 수 있습니다.");return;}const period=periods.find(item=>item.id===record.collectionId);if(!canCollect(period)){showToast("제출 마감 후에는 회수할 수 없습니다.");return;}if(!window.confirm("제출을 회수하고 작성 중 상태로 되돌리시겠습니까? 입력값과 증빙은 그대로 유지됩니다."))return;onChange(records.map(item=>item.id===record.id?{...item,status:"작성중" as RecordStatus,updatedAt:"방금 전"}:item),{action:"제출 회수",target:`${record.company} · ${record.source}`,detail:`${record.period} 활동자료 제출을 회수해 작성 중 상태로 되돌렸습니다.`});showToast("제출을 회수했습니다. 내용을 수정한 뒤 다시 제출할 수 있습니다.");};
  const remove=(record:ActivityRecord)=>{const period=periods.find(item=>item.id===record.collectionId);if(record.locked||record.status==="확정"||period?.status==="잠금"){showToast("확정 또는 잠금된 자료는 삭제할 수 없습니다.");return;}if(!window.confirm("이 활동자료를 삭제하시겠습니까?"))return;onChange(records.map(item=>item.id===record.id?{...item,active:false,updatedAt:"방금 전"}:item),{action:"자료 삭제",target:`${record.company} · ${record.source}`,detail:`${record.period} 활동자료를 삭제했습니다.`});showToast("활동자료를 삭제했습니다.");};
...
function activityChecks(record:ActivityRecord,records:ActivityRecord[],criteria:CollectionCriteria):OperationCheck[]{
  const checks:OperationCheck[]=[];
  const valueError=record.usage<0?"사용량은 0 이상이어야 합니다.":validateDataValue({value:record.usage,dataStatus:record.dataStatus,description:record.description});
  checks.push({title:"자료 구분·사용량",detail:valueError||DATA_STATUS_LABELS[dataValueStatus({value:record.usage,dataStatus:record.dataStatus})],severity:valueError?"error":"pass"});
  if(!Number.isFinite(record.factor)||record.factor<0)checks.push({title:"배출계수",detail:"배출계수는 0 이상 유한한 값이어야 합니다.",severity:"error"});
  const duplicate=records.some(item=>item.id!==record.id&&item.active!==false&&item.collectionId===record.collectionId&&item.company===record.company&&item.site===record.site&&item.period===record.period&&item.scope===record.scope&&item.category===record.category&&item.source===record.source);
  checks.push({title:"중복 입력",detail:duplicate?"같은 수집기간·사업장·기준월·배출원이 중복되었습니다.":"같은 조건으로 등록된 다른 자료가 없습니다.",severity:duplicate?"error":"pass"});
  const calculation=record.factorSnapshot?calculateEmission({usage:record.usage,unit:record.unit,period:record.period,factor:{...record.factorSnapshot,active:true}}):null;
  const expected=calculation?.ok?calculation.emissions:record.usage*record.factor/1000;
  if(calculation&&!calculation.ok)checks.push({title:"단위·적용기간",detail:calculation.errors.join(" "),severity:"error"});
  if(!Number.isFinite(record.emissions)||Math.abs(expected-record.emissions)>0.011)checks.push({title:"산정값 일관성",detail:"저장된 사용량·계수와 배출량이 일치하는지 확인해 주세요.",severity:"error"});
  else checks.push({title:"산정값 일관성",detail:"저장된 사용량·계수와 배출량이 일치합니다.",severity:"pass"});
  if(!record.factorSnapshot)checks.push({title:"기존 계수 기록",detail:"이전 자료에는 계수 출처·버전 기록이 없습니다. 저장된 계수값은 유지됩니다.",severity:"warning"});
  const previous=records.find(item=>item.active!==false&&item.company===record.company&&item.site===record.site&&item.scope===record.scope&&item.category===record.category&&item.source===record.source&&item.period===previousMonth(record.period));
  if(previous?.usage){const variance=(record.usage-previous.usage)/previous.usage*100;if(Math.abs(variance)>=criteria.variance)checks.push({title:"전월 급변",detail:"전월 대비 "+formatNumber(Math.abs(variance),1)+"% "+(variance>0?"증가":"감소")+"했습니다. "+(record.description?"입력 설명에서 변동 사유를 검토해 주세요.":"변동 사유가 입력되지 않았습니다."),severity:"warning"});}
  return checks;
}

function Review({records,periods,criteria,actor,onChange,showToast}:{records:ActivityRecord[];periods:CollectionPeriod[];criteria:CollectionCriteria;actor:string;onChange:(records:ActivityRecord[],auditInfo?:{action:string;target:string;detail:string})=>void;showToast:(m:string)=>void}){
  const activeRecords=records.filter(record=>record.active!==false);
  const handleAction=(id:number,action:"approve"|"reject"|"note"|"reopen",note:string)=>{
    const selected=activeRecords.find(record=>record.id===id);if(!selected)return;
    if(["마감","잠금"].includes(periods.find(period=>period.id===selected.collectionId)?.status??"")){showToast("수집기간을 다시 열고 서버 저장 완료 후 처리해 주세요.");return;}
    if(action==="reopen" && (selected.status!=="확정" || !note.trim()))return;
    if(action!=="note"&&action!=="reopen"&&selected.status!=="검토대기"){showToast("검토대기 자료만 처리할 수 있습니다.");return;}
    if(action==="approve"&&activityChecks(selected,records,criteria).some(check=>check.severity==="error")){showToast("검증 오류를 수정한 뒤 확정해 주세요.");return;}
    const at=new Date().toISOString();const label=action==="approve"?"자료 확정":action==="reopen"?"확정 취소":action==="reject"?"보완 요청":"검토 의견";
    onChange(records.map(record=>record.id===id?{...record,status:action==="approve"?"확정":(action==="reject"||action==="reopen")?"반려":record.status,locked:action==="approve"?criteria.lockConfirmed:(action==="reject"||action==="reopen")?false:record.locked,rejectionReason:(action==="reject"||action==="reopen")?note:action==="approve"?"":record.rejectionReason,reviewNote:note,reviewedAt:at,reviewedBy:actor,reviewHistory:[...(record.reviewHistory??[]),{at,actor,action:label,note}],updatedAt:nowLabel()}:record),{action:label,target:selected.company+" · "+selected.source,detail:note||selected.period+" 자료를 검토했습니다."});
    showToast("검토 결과를 서버에 저장 중입니다. 상단 저장 상태를 확인해 주세요.");
  };
  return <><PageHeader eyebrow="REVIEW & APPROVAL" title="검토·승인 작업함" description="사업장별 제출값과 계산 근거를 확인하고, 행별 검토 의견과 보완 요청을 남깁니다."/><OperationsWorkbench rows={activeRecords.map(record=>({ ...record,title:record.category+" · "+record.source,value:formatNumber(record.emissions,2)+" tCO₂e",dataStatus:DATA_STATUS_LABELS[dataValueStatus({value:record.usage,dataStatus:record.dataStatus})],locked:["마감","잠금"].includes(periods.find(period=>period.id===record.collectionId)?.status??""),resubmitted:record.reviewHistory?.some(item=>["보완 요청","확정 취소"].includes(item.action)),facts:[{label:"활동량",value:formatNumber(record.usage,4)+" "+record.unit},{label:"저장된 배출계수",value:String(record.factor)},{label:"계수 식별자",value:record.factorId||"기존 자료 · 기록 없음"},{label:"계수 적용 기준",value:record.factorSnapshot?[record.factorSnapshot.year+"년",record.factorSnapshot.authority,record.factorSnapshot.factorUnit,"버전 "+record.factorSnapshot.revision,"유효기간 "+(record.factorSnapshot.validFrom||"시작 제한 없음")+" ~ "+(record.factorSnapshot.validTo||"종료 제한 없음")].join(" · "):"기존 저장값 유지 · 신규 입력부터 계수 버전 보존"}],checks:activityChecks(record,records,criteria),history:record.reviewHistory}))} onAction={handleAction}/></>;
}

function DataQuality({records,periods,criteria,actor,onChange,onOpenRecord,showToast}:{records:ActivityRecord[];periods:CollectionPeriod[];criteria:CollectionCriteria;actor:string;onChange:(records:ActivityRecord[],auditInfo?:{action:string;target:string;detail:string})=>void;onOpenRecord:(record:ActivityRecord)=>void;showToast:(m:string)=>void}){
  const activeRecords=records.filter(record=>record.active!==false);
  const issues=activeRecords.flatMap(record=>activityChecks(record,records,criteria).filter(check=>check.severity!=="pass").map(check=>({key:record.id+":"+check.title,recordId:record.id,severity:check.severity as "error"|"warning",rule:check.title,detail:check.detail,company:record.company,site:record.site,period:record.period,title:record.source,owner:record.owner,editable:!record.locked&&!["확정","검토대기"].includes(record.status)&&canCollect(periods.find(period=>period.id===record.collectionId)),fingerprint:JSON.stringify([record.usage,record.factor,record.emissions,record.unit,record.dataStatus,record.description,check.detail]),resolution:record.qualityResolutions?.[check.title],resolutionLocked:["마감","잠금"].includes(periods.find(period=>period.id===record.collectionId)?.status??"")})));
  return <><PageHeader eyebrow="DATA QUALITY" title="입력 오류·확인 사항" description={"사용량·산식·중복·전월 급변(±"+criteria.variance+"%)을 점검하고, 관련 입력 수정과 확인 사유를 기록합니다."}/><OperationsQuality issues={issues} checkedCount={activeRecords.length} onOpenRecord={id=>{const record=records.find(item=>item.id===id);if(record)onOpenRecord(record)}} onResolve={(issue,reason,status)=>{if(issue.resolutionLocked){showToast("마감 또는 잠금된 수집기간입니다. 기간을 다시 열고 서버 저장을 완료한 후 확인 내용을 수정해 주세요.");return;}const resolution:QualityResolution={reason,status,updatedAt:new Date().toISOString(),actor,fingerprint:issue.fingerprint};onChange(records.map(record=>record.id===issue.recordId?{...record,qualityResolutions:{...record.qualityResolutions,[issue.rule]:resolution}}:record),{action:"품질 문제 검토",target:issue.company+" · "+issue.title,detail:issue.rule+" / "+status+": "+reason});showToast("확인 내용을 서버에 저장 중입니다.");}}/></>;
}
...
function Inventory({records,targets,organizationNames,onNavigate,showToast}:{records:ActivityRecord[];targets:ReductionTarget[];organizationNames:string[];onNavigate:(view:View)=>void;showToast:(m:string)=>void}){
  ...
  const exportData=()=>{downloadCsv("sems2_ghg_inventory.csv",["자료 기준월","법인","사업장","Scope","활동자료","배출원","사용량","단위","배출량(tCO2e)","상태"],rows.map(r=>[r.period,r.company,r.site,r.scope,r.category,r.source,r.usage,r.unit,r.emissions,r.status]));showToast("산정 내역을 내려받았습니다.");};
...
function TargetsAndPlans({targets,plans,records,organizations,canManage,onTargetsChange,onPlansChange,addAudit,showToast}:{targets:ReductionTarget[];plans:ReductionPlan[];records:ActivityRecord[];organizations:Record<string,string[]>;canManage:boolean;onTargetsChange:(items:ReductionTarget[])=>void;onPlansChange:(items:ReductionPlan[])=>void;addAudit:(action:string,target:string,detail:string,actor?:string)=>void;showToast:(message:string)=>void}){
  ...
  return <><PageHeader eyebrow="TARGET & ACTION PLAN" title="감축목표·이행계획" description="확정 인벤토리를 기준으로 목표와 실행과제, 이행실적을 확인합니다.">{canManage&&<><button className="secondary-button" onClick={()=>setTargetModal("new")}><Icon name="target" size={17}/>새 목표 설정</button><button className="primary-button" onClick={()=>selected?setPlanModal("new"):showToast("감축목표를 먼저 설정해 주세요.")} disabled={!selected}><Icon name="plus" size={17}/>감축과제 등록</button></>}</PageHeader>
    ...
    <section className={`target-workspace ${canManage?"":"read-only"}`}><aside className="card target-list"><CardHeader title="감축목표" subtitle="목표를 선택하면 산정근거와 이행계획이 연결됩니다." action={canManage?<button className="outline-small" onClick={()=>setTargetModal("new")}><Icon name="plus" size={14}/>추가</button>:undefined}/><div>{targets.map(target=><button key={target.id} className={selected?.id===target.id?"active":""} onClick={()=>setSelectedId(target.id)}><span className={`target-state ${target.status}`}>{target.status}</span><strong>{target.name}</strong><p>{target.company} · {target.scopes.join("·")}</p><div><span>{target.baselineYear} → {target.targetYear}</span><b>-{target.reductionRate}%</b></div></button>)}{!targets.length&&<div className="empty-state compact"><Icon name="target"/><strong>등록된 감축목표가 없습니다.</strong></div>}</div></aside>
...
function Evidence({items,onChange,showToast}:{items:EvidenceItem[];onChange:(x:EvidenceItem[])=>void;showToast:(m:string)=>void}){
  const {canWrite,canManage}=useSemsAuth();
  ...
  const remove=async(item:EvidenceItem)=>{if(!canManage&&["승인","만료"].includes(item.status)){showToast("승인 또는 만료 처리된 증빙은 관리자만 변경할 수 있습니다.");return;}if(!window.confirm("이 증빙자료를 라이브러리에서 삭제하시겠습니까?"))return;if(item.storagePath&&supabase)await supabase.storage.from("sems2-evidence").remove([item.storagePath]);onChange(items.filter(row=>row.id!==item.id));setEditing(null);showToast("증빙자료를 삭제했습니다.");};
  ...
  return <><PageHeader eyebrow="EVIDENCE LIBRARY" title="증빙자료 라이브러리" description="한 번 등록한 원본 증빙을 여러 지표·평가·공시 항목에 연결하고 버전과 유효기간을 관리합니다."><button className="secondary-button" onClick={exportList}><Icon name="download" size={17}/>목록 내보내기</button>{canWrite&&<button className="primary-button" onClick={()=>setEditing("new")}><Icon name="upload" size={17}/>증빙자료 등록</button>}</PageHeader>
    ...
    {editing&&<EvidenceForm item={editing==="new"?null:editing} organizations={Object.keys(readOrganizations())} onClose={()=>setEditing(null)} onSave={save} onDelete={editing==="new"||!canManage?undefined:()=>void remove(editing)}/>}
  </>;
}
...
function EvidenceForm({item,organizations,onClose,onSave,onDelete}:{item:EvidenceItem|null;organizations:string[];onClose:()=>void;onSave:(item:EvidenceItem)=>void;onDelete?:()=>void}){
  const supabase=getSupabaseBrowserClient();
  const {profile,canManage}=useSemsAuth();
  ...
  const submit=async(event:FormEvent)=>{event.preventDefault();if(!canManage&&item&&["승인","만료"].includes(item.status)){setError("승인 또는 만료 처리된 증빙은 관리자만 수정할 수 있습니다.");return;}setUploading(true);setError("");try{let fileName=form.fileName;let storagePath=form.storagePath;if(file){if(!supabase)throw new Error("Supabase Storage 연결 정보가 없습니다.");if(!profile.organization_id&&!canManage)throw new Error("소속 법인이 지정되지 않았습니다.");const safeName=file.name.replace(/[^a-zA-Z0-9._-]/g,"_");const folder=profile.organization_id?`${profile.organization_id}/${profile.id}`:`admin/${profile.id}`;storagePath=`${folder}/${Date.now()}-${safeName}`;const {error:uploadError}=await supabase.storage.from("sems2-evidence").upload(storagePath,file,{upsert:false});if(uploadError)throw uploadError;fileName=file.name;}onSave({...form,organization:canManage?form.organization:(profile.organization?.name??""),status:canManage?form.status:"검토중",fileName,storagePath,updatedAt:nowLabel()});}catch(uploadError){setError(uploadError instanceof Error?uploadError.message:"파일을 업로드하지 못했습니다.");}finally{setUploading(false);}};
...
const EXCLUDED_GHG_METRIC_CODES=new Set(["ENV-GHG-S1","ENV-GHG-S2","ENV-GHG-S3"]);
const EXCLUDED_GHG_METRIC_IDS=new Set([
  ...GRI_WORKBOOK_EXCLUDED_INDICATOR_IDS,
  ...COMPLILAW_QUANTITATIVE_INDICATORS.filter(item=>EXCLUDED_GHG_METRIC_CODES.has(item.code)).map(item=>item.id),
]);
...
function normalizeMetricRequests(source:MetricRequest[],indicatorIdMap:Map<number,number>):MetricRequest[]{
  return source.flatMap(request=>{
    ...
    const taskKeys=request.taskKeys?.flatMap(key=>{
      ...
      return [collectionTaskKey(parsed.company,targetId,parsed.period,parsed.site)];
    });
...
function normalizeMetricSubmissions(source:MetricSubmission[],indicatorIdMap:Map<number,number>,indicators:Indicator[]):MetricSubmission[]{
  ...
  const normalized=source.filter(submission=>submission.active!==false&&!EXCLUDED_GHG_METRIC_IDS.has(submission.indicatorId)).flatMap<MetricSubmission>(submission=>{
  ...
  normalized.forEach(submission=>{
    ...
    const key=`${submission.requestId}::${submission.indicatorId}::${submission.company}::${submission.site}::${submission.period}`;
  ...
  return [...source.filter(submission=>submission.active===false), ...merged.values()];
}
...
function tasksAtSites<T extends string|number>(tasks:CollectionTask<T>[],organizations:Record<string,string[]>,requested?:Record<string,string[]>):CollectionTask<T>[] {
  return tasks.flatMap(task=>task.site?[task]:(requested?.[task.company]??organizations[task.company]??[]).length?(requested?.[task.company]??organizations[task.company]).map(site=>({...task,site,key:collectionTaskKey(task.company,task.targetId,task.period,site)})):[task]);
}
...
function MetricCollection({mode,requests,submissions,indicators,organizations,canWrite,canManage,currentOrganization,defaultOwner,defaultDepartment,onRequestsChange,onSubmissionsChange,onIndicatorsChange,addAudit,showToast}:{mode:"input"|"request"|"review";requests:MetricRequest[];submissions:MetricSubmission[];indicators:Indicator[];organizations:Record<string,string[]>;canWrite:boolean;canManage:boolean;currentOrganization:string;defaultOwner:string;defaultDepartment:string;onRequestsChange:(items:MetricRequest[])=>void;onSubmissionsChange:(items:MetricSubmission[])=>void;onIndicatorsChange:(items:Indicator[])=>void;addAudit:(action:string,target:string,detail:string,actor?:string)=>void;showToast:(message:string)=>void}){
  ...
  const [submissionModal,setSubmissionModal]=useState<{request:MetricRequest;indicator:Indicator;company:string;site?:string;period:string;submission?:MetricSubmission}|null>(null);
  ...
  const expectedRows=selected?tasksAtSites(buildMetricCollectionTasks(selected,indicators),organizations,selected.sitesByCompany).filter(task=>visibleCompanies.includes(task.company)).map(task=>({company:task.company,site:task.site,period:task.period,indicator:indicators.find(indicator=>indicator.id===task.targetId),submission:submissions.find(item=>item.requestId===selected.id&&item.company===task.company&&(!task.site||item.site===task.site)&&item.indicatorId===task.targetId&&item.period===task.period)})).filter(row=>row.indicator):[];
  ...
  const saveRequest=(request:MetricRequest)=>{
    ...
    if(request.status==="마감"&&requests.find(item=>item.id===request.id)?.status!=="마감"){const issues=getWorkspaceClosingIssues({kind:"metric",request,rows:submissions,indicators,organizations});if(issues.length){setClosingIssues(issues.map(issue=>issue.message));return;}}
    const saved={...request,sitesByCompany:request.sitesByCompany??Object.fromEntries(request.companies.map(company=>[company,organizations[company]??[]])),id:exists?request.id:`MR-${Date.now()}`,taskKeys,updatedAt:nowLabel()};
  ...
  const deleteRequest=(request:MetricRequest)=>{
    const linked=submissions.filter(item=>item.requestId===request.id);
    if(linked.some(item=>item.status==="확정")){showToast("확정 제출값이 연결된 요청은 삭제할 수 없습니다. 상태를 마감으로 변경해 이력을 보존하세요.");return;}
    if(!window.confirm("이 수집 요청과 연결된 미확정 제출 데이터를 삭제하시겠습니까?"))return;
    ...
    addAudit("정량데이터 수집 요청 삭제",request.title,"확정 제출값이 없는 수집 요청과 미확정 제출값을 삭제했습니다.");showToast("수집 요청을 삭제했습니다.");
  };
  const addTemplates=()=>{
  ...
  const saveSubmission=(submission:MetricSubmission)=>{
    if(!canCollect(requests.find(item=>item.id===submission.requestId))){showToast("제출이 마감되었습니다. 관리자에게 제출기한 연장을 요청해 주세요.");return;}
    const exists=submissions.some(item=>item.id===submission.id);
    if(submissions.some(item=>item.id!==submission.id&&item.requestId===submission.requestId&&item.company===submission.company&&item.site===submission.site&&item.indicatorId===submission.indicatorId&&item.period===submission.period)){showToast("같은 사업장·지표·기간의 제출값이 이미 있습니다. 기존 자료를 수정해 주세요.");return;}
  ...
  const deleteSubmission=(submission:MetricSubmission)=>{
    if(!canWrite || !(canManage || submission.company===currentOrganization) || !canCollect(requests.find(item=>item.id===submission.requestId)) || !["작성중","반려"].includes(submission.status))return;
    if(!window.confirm("이 정량데이터를 삭제하시겠습니까? 입력 목록과 집계에서 제외되며 변경 이력은 보존됩니다."))return;
    onSubmissionsChange(submissions.map(item=>item.id===submission.id?{...item,active:false,updatedAt:nowLabel()}:item));
    addAudit("정량데이터 삭제",submission.company+" · "+(indicators.find(item=>item.id===submission.indicatorId)?.name??"ESG 지표"),submission.period+" 자료를 삭제했습니다.");
    showToast("삭제 내용을 서버에 저장 중입니다. 상단 저장 상태를 확인해 주세요.");
  };
  ...
  return <><PageHeader eyebrow={isReview?"ESG DATA REVIEW":mode==="request"?"ESG DATA REQUEST":"ESG DATA INPUT"} title={isReview?"기타 ESG 데이터 검토·승인":mode==="request"?"기타 ESG 수집 요청":"기타 ESG 데이터 입력"} description={isReview?"제출된 정량데이터와 증빙을 확인한 뒤 확정하거나 반려합니다.":mode==="request"?"대상 법인·지표와 수집 기간, 제출 마감을 설정합니다.":"배정된 환경·사회·지배구조 정량데이터를 입력하고 검토 요청으로 제출합니다."}>{mode==="request"&&canManage&&<button className="secondary-button" onClick={addTemplates}><Icon name="list" size={17}/>엑셀·GRI 지표 복원</button>}{selected&&mode!=="request"&&<button className="secondary-button" onClick={exportRows}><Icon name="download" size={17}/>수집현황 내보내기</button>}{mode==="request"&&canManage&&<button className="primary-button" onClick={()=>setRequestModal("new")} disabled={!indicators.length}><Icon name="plus" size={17}/>수집 요청 추가</button>}</PageHeader>
    {workspaceTab==="periods"?<>
      ...
      <section className="period-grid">{requests.map(request=>{const rows=submissions.filter(item=>item.requestId===request.id);const tasks=tasksAtSites(buildMetricCollectionTasks(request,indicators),organizations,request.sitesByCompany);const submitted=rows.filter(item=>["검토대기","확정"].includes(item.status)).length;const confirmed=rows.filter(item=>item.status==="확정").length;const total=tasks.length;const cycleCounts=countTasksByCycle(tasks);const requestCompletion=total?Math.round(confirmed/total*100):0;return <article className="card period-card" key={request.id}><div className="period-card-top"><div><StatusBadge status={request.status==="검토중"?"제출 마감 · 검토중":request.status}/><h2>{request.title}</h2><p>{request.description||"별도 요청사항이 없습니다."}</p></div></div><div className="period-dates"><div><span>귀속기간</span><strong>{request.periodFrom===request.periodTo?request.periodFrom:`${request.periodFrom} ~ ${request.periodTo}`}</strong></div><div><span>제출마감</span><strong style={koreanHolidayName(request.dueDate)?{color:"#d32f2f"}:undefined}>{request.dueDate}{koreanHolidayName(request.dueDate)&&<em style={{display:"block",marginTop:4,fontStyle:"normal",fontSize:12,fontWeight:700,color:"#d32f2f"}}>{koreanHolidayName(request.dueDate)}</em>}</strong></div><div><span>최근 변경</span><strong>{request.updatedAt}</strong></div></div><div className="period-targets"><span>{request.companies.length}개 법인</span><span>{request.indicatorIds.length}개 지표</span>{(Object.entries(cycleCounts) as [CollectionCycle,number][]).filter(([,count])=>count>0).map(([cycle,count])=><span key={cycle}>{cycle} {count}건</span>)}</div><div className="period-progress"><div><span>대상 {total}건 · 제출 {submitted}건 · 확정 {confirmed}건</span><strong>{requestCompletion}%</strong></div><div className="progress-track"><span style={{width:`${requestCompletion}%`}}/></div></div>{canManage&&<div className="period-actions"><button className="secondary-button compact" onClick={()=>setRequestModal(request)}><Icon name="edit" size={14}/>설정 수정</button><button className="danger-button compact" onClick={()=>deleteRequest(request)}><Icon name="trash" size={14}/>삭제</button></div>}</article>})}</section>
...
function MetricReview({requests,submissions,indicators,actor,onChange,addAudit,showToast}:{requests:MetricRequest[];submissions:MetricSubmission[];indicators:Indicator[];actor:string;onChange:(rows:MetricSubmission[])=>void;addAudit:(action:string,target:string,detail:string)=>void;showToast:(m:string)=>void}){
  const checksFor=(row:MetricSubmission):OperationCheck[]=>{const error=row.value<0?"제출값은 0 이상이어야 합니다.":validateDataValue({value:row.value,dataStatus:row.dataStatus,description:row.description});const indicator=indicators.find(item=>item.id===row.indicatorId);return [{title:"제출값·자료 구분",detail:error||"자료 구분에 맞는 값입니다.",severity:error?"error":"pass"},{title:"지표 정의",detail:indicator?.definition||"지표 정의가 등록되지 않았습니다.",severity:indicator?"pass":"error"},{title:"산정 기준",detail:row.description||"입력자가 남긴 산정 설명이 없습니다.",severity:row.description?"pass":"warning"}];};
  return <><PageHeader eyebrow="ESG DATA REVIEW" title="기타 ESG 검토·승인 작업함" description="지표 정의, 상세 입력값과 검증 결과를 대조하고 검토 의견을 함께 보존합니다."/><OperationsWorkbench rows={submissions.map(row=>{const indicator=indicators.find(item=>item.id===row.indicatorId);return {...row,locked:["마감","잠금"].includes(requests.find(item=>item.id===row.requestId)?.status??""),title:indicator?.name??"정의가 없는 지표",value:indicator&&metricTemplateOf(indicator)==="FIXED"?(row.detailRows?.length??0)+"개 세부값":formatNumber(row.value,2)+" "+row.unit,dataStatus:row.detailRows?.length&&row.dataStatus==="actual"?DATA_STATUS_LABELS.actual:DATA_STATUS_LABELS[dataValueStatus({value:row.value,dataStatus:row.dataStatus})],resubmitted:row.reviewHistory?.some(item=>["보완 요청","확정 취소"].includes(item.action)),facts:[{label:"지표 코드",value:indicator?.code??""},{label:"산식",value:indicator?.formula??""},{label:"집계 방식",value:(indicator?.aggregation??"합계")+" · 조직 "+(indicator?.organizationAggregation??"합계")},{label:"수집 요청",value:requests.find(item=>item.id===row.requestId)?.title??""},{label:"보고 기준",value:indicator?.frameworks.join(", ")??""},{label:"경계",value:indicator?.boundary??""}],checks:checksFor(row),history:row.reviewHistory,detail:row.detailRows?.length?<div className="operations-description"><h3>상세 입력값</h3><div className="table-scroll"><table className="data-table"><tbody>{row.detailRows.map((detail,index)=><tr key={detail.id}><th>{index+1}</th><td>{Object.entries(detail.values).filter(([key])=>key!=="detailKey").map(([key,value])=><span key={key}>{indicator?METRIC_TEMPLATE_FIELDS[metricTemplateOf(indicator)].find(field=>field.key===key)?.label??({label:"항목",unit:"단위",amount:"값"} as Record<string,string>)[key]??key:key}: {String(value)} </span>)}</td></tr>)}</tbody></table></div></div>:undefined}})} onAction={(id,action,note)=>{const row=submissions.find(item=>item.id===id);if(!row)return;if(["마감","잠금"].includes(requests.find(item=>item.id===row.requestId)?.status??"")){showToast("수집기간을 다시 열고 서버 저장 완료 후 처리해 주세요.");return;}if(action==="reopen"&&(row.status!=="확정"||!note.trim()))return;if(action!=="note"&&action!=="reopen"&&row.status!=="검토대기")return;if(action==="approve"&&checksFor(row).some(check=>check.severity==="error")){showToast("검증 오류를 먼저 수정해 주세요.");return;}const at=new Date().toISOString();const label=action==="approve"?"정량데이터 확정":action==="reopen"?"확정 취소":action==="reject"?"보완 요청":"검토 의견";onChange(submissions.map(item=>item.id===id?{...item,status:action==="approve"?"확정":(action==="reject"||action==="reopen")?"반려":item.status,rejectionReason:(action==="reject"||action==="reopen")?note:action==="approve"?"":item.rejectionReason,reviewNote:note,reviewedAt:at,reviewedBy:actor,reviewHistory:[...(item.reviewHistory??[]),{at,actor,action:label,note}],updatedAt:nowLabel()}:item));addAudit(label,row.company+" · "+(indicators.find(item=>item.id===row.indicatorId)?.name??"ESG 지표"),note||"제출값과 산정 기준을 검토했습니다.");showToast("검토 결과를 서버에 저장 중입니다.");}}/></>;
}
...
function MetricSubmissionForm({context,organizations,defaultOwner,defaultDepartment,onClose,onSave}:{context:{request:MetricRequest;indicator:Indicator;company:string;site?:string;period:string;submission?:MetricSubmission};organizations:Record<string,string[]>;defaultOwner:string;defaultDepartment:string;canManage:boolean;onClose:()=>void;onSave:(submission:MetricSubmission)=>void}){
  const review=useSubmissionReview<MetricSubmission>(onSave);
  ...
  const [form,setForm]=useState<MetricSubmission>(existing??{id:0,requestId:context.request.id,indicatorId:context.indicator.id,company:context.company,site:context.site??organizations[context.company]?.[0]??"",period:context.period,value:0,dataStatus:"missing",unit:context.indicator.unit,owner:defaultOwner,department:defaultDepartment,evidence:"",description:"",status:"작성중",detailRows:template==="GENERAL"?undefined:initialRows,employeeCount:template==="TRAINING"?0:undefined,updatedAt:"방금 전"});
  const [rows,setRows]=useState<MetricDetailRow[]>(initialRows);
  const [error,setError]=useState("");
  const isRatio=context.indicator.organizationAggregation==="비율"||context.indicator.unit.includes("%")||context.indicator.unit.includes("/");
  const ratioScale=context.indicator.ratioMultiplier??(context.indicator.unit.includes("%")?100:1);
  const patch=(value:Partial<MetricSubmission>)=>{setForm(current=>{const next={...current,...value};if(isRatio&&next.dataStatus!=="not_applicable"&&Number.isFinite(next.numerator)&&Number.isFinite(next.denominator)&&next.denominator!>0){next.value=next.numerator!/next.denominator!*ratioScale;if(next.dataStatus==="missing")next.dataStatus="actual";}return next;});setError("");};
  const syncRows=(nextRows:MetricDetailRow[])=>{setRows(nextRows);setForm(current=>({...current,detailRows:nextRows,dataStatus:current.dataStatus==="estimated"?"estimated":"actual",value:template==="FIXED"?0:calculateMetricDetailTotal(template,nextRows)}));setError("");};
  ...
  const submit=(event:FormEvent)=>{
    event.preventDefault();
    const valueError=validateDataValue({value:form.value,dataStatus:form.dataStatus,description:form.description});if(valueError){setError(valueError);return;}
    if(!Number.isFinite(form.value)||form.value<0){setError("제출값은 0 이상 숫자여야 합니다.");return;}
    if(isRatio&&form.dataStatus!=="not_applicable"&&(!Number.isFinite(form.numerator)||!Number.isFinite(form.denominator)||form.denominator!<=0)){setError("비율 지표의 분자와 0보다 큰 분모를 입력해 주세요.");return;}
    ...
    review.save({...form,dataStatus:template==="FIXED"&&form.dataStatus==="actual"?"actual":dataValueStatus({value:form.value,dataStatus:form.dataStatus}),status:submissionIntent(event),rejectionReason:submissionIntent(event)==="검토대기"?"":form.rejectionReason,detailRows:template==="GENERAL"?undefined:rows,value:form.dataStatus==="not_applicable"||form.dataStatus==="zero"?0:isRatio?form.value:template==="GENERAL"?form.value:template==="FIXED"?0:calculateMetricDetailTotal(template,rows)}, submissionIntent(event));
  };
  const fields=METRIC_TEMPLATE_FIELDS[template];
  return <Overlay title={`${context.indicator.name} 입력`} eyebrow="METRIC SUBMISSION" description={`${context.company} · ${context.period} · ${context.indicator.cycle} 수집 · ${METRIC_TEMPLATE_LABELS[template]} 양식`} onClose={onClose}><GuidedForm onSubmit={submit}>
    <div className="form-section"><h3><span>1</span>지표와 대상</h3><div className="form-grid"><label>법인<input value={form.company} readOnly/></label><label>사업장<SimpleSelect disabled={Boolean(context.site)} value={form.site} onChange={event=>patch({site:event.target.value})}>{(organizations[form.company]??[]).map(site=><option key={site}>{site}</option>)}</SimpleSelect></label><label>지표명<input value={context.indicator.name} readOnly/></label><label>입력 기간<input type="month" value={form.period} readOnly className="readonly-input"/></label></div></div>
    <div className="form-section"><h3><span>2</span>{template==="GENERAL"?"값 입력":`${METRIC_TEMPLATE_LABELS[template]} 상세 입력`}</h3><DataStatusFields value={form.value} status={form.dataStatus} description={form.description} onChange={dataStatus=>{if(dataStatus==="zero")setRows(current=>current.map(row=>({...row,values:Object.fromEntries(Object.entries(row.values).map(([key,value])=>[key,typeof value==="number"?0:value]))})));patch({dataStatus,...(["zero","not_applicable"].includes(dataStatus)?{value:0,numerator:dataStatus==="zero"?0:form.numerator}:{})});}}/><fieldset className="operations-detail-fieldset" disabled={form.dataStatus==="zero"||form.dataStatus==="not_applicable"}>{template==="GENERAL"?<div className="form-grid"><label>제출값<div className="input-unit"><input type="number" min="0" step="any" value={form.dataStatus==="missing"?"":form.value} disabled={form.dataStatus==="not_applicable"||form.dataStatus==="zero"} onChange={event=>patch({value:Number(event.target.value),dataStatus:event.target.value===""?"missing":form.dataStatus==="estimated"?"estimated":"actual"})} readOnly={isRatio} required/><span>{form.unit}</span></div></label></div>:template==="FIXED"?<FixedMetricDetailEditor rows={rows} onChange={(rowId,value)=>updateRow(rowId,"amount",value)}/>:<div className="metric-detail-editor">{template==="TRAINING"&&<div className="training-headcount"><label><span>임직원 총원</span><div className="input-unit"><input type="number" min="1" step="1" value={form.employeeCount||""} onChange={event=>patch({employeeCount:Number(event.target.value)})} required/><span>명</span></div></label><p>총 교육시간을 임직원 총원으로 나눠 1인당 교육시간을 자동 계산합니다.</p></div>}<div className="metric-detail-editor-head"><div><span className={`metric-template-badge ${template.toLowerCase()}`}>{METRIC_TEMPLATE_LABELS[template]}</span><p>상세행을 추가하면 {fields.find(field=>field.aggregate)?.label??"값"}이 자동 합산됩니다.</p></div><button type="button" className="outline-small" onClick={addRow}><Icon name="plus" size={14}/>행 추가</button></div><div className="metric-detail-rows">{rows.map((row,index)=><div className="metric-detail-row" key={row.id}><span className="metric-row-number">{index+1}</span><div className="metric-detail-fields" style={{gridTemplateColumns:`repeat(${Math.min(fields.length,5)}, minmax(130px, 1fr))`}}>{fields.map(field=><label key={field.key}><span>{field.label}</span>{field.type==="select"?<SimpleSelect value={String(row.values[field.key]??field.options?.[0]??"")} onChange={event=>updateRow(row.id,field.key,event.target.value)}>{field.options?.map(option=><option key={option}>{option}</option>)}</SimpleSelect>:<input type={field.type} min={field.type==="number"?"0":undefined} step={field.type==="number"?"any":undefined} value={row.values[field.key]??""} readOnly={field.readOnly} placeholder={field.placeholder} onChange={event=>updateRow(row.id,field.key,field.type==="number"?Number(event.target.value):event.target.value)}/>}</label>)}</div><button type="button" className="metric-row-remove" onClick={()=>removeRow(row.id)} aria-label={`${index+1}번 상세행 삭제`}><Icon name="trash" size={15}/></button></div>)}</div><div className="metric-derived-totals"><div><span>자동 집계 제출값</span><strong>{formatNumber(calculateMetricDetailTotal(template,rows),2)} <small>{form.unit}</small></strong><em>{rows.length}개 상세행 합계</em></div>{template==="WASTE"&&<div><span>재활용량</span><strong>{formatNumber(calculateRecycledWaste(rows),2)} <small>{form.unit}</small></strong><em>처리방법 ‘재활용’ 합계</em></div>}{template==="TRAINING"&&<div><span>1인당 교육시간</span><strong>{form.employeeCount?formatNumber(calculateMetricDetailTotal(template,rows)/form.employeeCount,2):"-"} <small>시간/인</small></strong><em>총 교육시간 ÷ 임직원 총원</em></div>}</div></div>}</fieldset>{(context.indicator.organizationAggregation==="비율"||context.indicator.unit.includes("%")||context.indicator.unit.includes("/"))&&<div className="form-grid operations-ratio-fields"><label>집계 분자<input type="number" step="any" value={form.numerator??""} onChange={event=>patch({numerator:event.target.value===""?undefined:Number(event.target.value)})}/></label><label>집계 분모<input type="number" min="0" step="any" value={form.denominator??""} onChange={event=>patch({denominator:event.target.value===""?undefined:Number(event.target.value)})}/><small>그룹 비율은 조직별 분자·분모의 합으로 다시 계산합니다.</small></label></div>}<div className="form-grid metric-submission-meta"><label>담당자<input value={form.owner} onChange={event=>patch({owner:event.target.value})} required/></label><label>담당 부서<input value={form.department} onChange={event=>patch({department:event.target.value})} required/></label><label className="full-span textarea-label">산정 기준·변동 사유<textarea value={form.description} onChange={event=>patch({description:event.target.value})} placeholder={context.indicator.formula||"산정 기준과 특이사항을 적어 주세요."}/></label></div></div>
    <div className="form-section"><h3><span>3</span>증빙자료 <small>(선택)</small></h3><label className="upload-zone"><input type="file" accept=".pdf,.xlsx,.xls,.csv,.jpg,.jpeg,.png" onChange={event=>{const file=event.target.files?.[0];if(!file)return;if(file.size>20*1024*1024){setError("증빙파일은 20MB 이하만 선택할 수 있습니다.");event.target.value="";return;}patch({evidence:file.name})}}/><span className="upload-icon"><Icon name="upload"/></span>{form.evidence?<><strong>{form.evidence}</strong><small>원본 파일명과 연결정보가 저장됩니다.</small></>:<><strong>증빙자료가 있으면 선택하세요.</strong><small>{context.indicator.evidenceExample||"선택사항 · PDF, XLSX, CSV, JPG, PNG · 최대 20MB"}</small></>}</label>{form.rejectionReason&&<div className="rejection-note"><Icon name="alert" size={16}/><div><strong>이전 보완 요청</strong><p>{form.rejectionReason}</p></div></div>}{error&&<p className="form-error"><Icon name="alert" size={14}/>{error}</p>}</div><div className="modal-footer"><button type="button" className="secondary-button" onClick={onClose}>취소</button><SubmissionButtons rejected={existing?.status==="반려"}/></div>
  </GuidedForm>{review.dialog}</Overlay>;
}
...
function ReportBuilder({reports,records,metricSubmissions,targets,indicators,standards,organizationNames,canManage,onChange,addAudit,showToast}:{reports:SustainabilityReport[];records:ActivityRecord[];metricSubmissions:MetricSubmission[];targets:ReductionTarget[];indicators:Indicator[];standards:DisclosureStandard[];organizationNames:string[];canManage:boolean;onChange:(items:SustainabilityReport[])=>void;addAudit:(action:string,target:string,detail:string,actor?:string)=>void;showToast:(message:string)=>void}){
  ...
  return <><PageHeader eyebrow="SUSTAINABILITY REPORT" title="지속가능경영보고서 작성" description="SEMS에서 확정한 데이터와 ESG 지표를 원고에 연결하고, 목차별 본문 작성부터 공시기준 대응·미리보기·PDF 출력까지 관리합니다.">{selected&&<button className="secondary-button" onClick={()=>window.print()}><Icon name="download" size={17}/>PDF 인쇄</button>}{canManage&&<button className="primary-button" onClick={()=>setEditing("new")}><Icon name="plus" size={17}/>새 보고서</button>}</PageHeader>
    {!reports.length?<section className="card report-empty"><div className="empty-state"><Icon name="edit"/><strong>작성 중인 지속가능경영보고서가 없습니다.</strong><p>기본 목차가 포함된 새 보고서를 만든 뒤 SEMS 데이터를 연결해 주세요.</p>{canManage&&<button className="primary-button" onClick={()=>setEditing("new")}><Icon name="plus" size={16}/>첫 보고서 만들기</button>}</div></section>:selected&&<><section className="report-summary card"><div className="report-summary-main" style={{borderColor:selected.primaryColor}}><span style={{background:selected.primaryColor}}>{selected.year}</span><div><div className="report-meta-line"><StatusBadge status={selected.status}/><em>{selected.organization}</em><em>{selected.reportingPeriod}</em></div><h2>{selected.title}</h2><p>{selected.frameworks.join(" · ")} 기준 · 최근 저장 {selected.updatedAt}</p></div></div><div className="report-summary-stat"><span>목차</span><strong>{selected.pages.length}<small>개</small></strong></div><div className="report-summary-stat"><span>원고 진행률</span><strong>{completion}<small>%</small></strong><div className="report-progress"><i style={{width:`${completion}%`,background:selected.primaryColor}}/></div></div><div className="report-summary-actions">{canManage&&<button className="secondary-button compact" onClick={()=>setEditing(selected)}><Icon name="settings" size={15}/>기본정보</button>}{canManage&&<button className="primary-button compact" onClick={saveCheckpoint}><Icon name="check" size={15}/>원고 저장</button>}</div></section>
      ...
      {tab==="standards"&&<ReportStandards report={selected} indicators={indicators} standards={standards} submissions={metricSubmissions}/>}
...
function annualMetricValue(indicator:Indicator,rows:MetricSubmission[],year:number){
  return aggregateMetric(indicator,rows.filter(row=>row.indicatorId===indicator.id),{year}).value;
}

function annualMetricDetailValue(indicator:Indicator,rows:MetricSubmission[],year:number,detail:GriWorkbookDetailSeed){
  const values=rows.filter(row=>row.indicatorId===indicator.id).map(row=>{const raw=row.detailRows?.find(item=>String(item.values.detailKey)===detail.key)?.values.amount;return {...row,value:raw===undefined||raw===""?null:Number(raw)};});
  return aggregateMetric({aggregation:detail.aggregation??indicator.aggregation,organizationAggregation:indicator.organizationAggregation,unit:detail.unit},values,{year}).value;
}
...
function ReportDataChart({source,years,report,records,metricSubmissions,targets,indicators,chartType}:{source:ReportDataSource;years:number[];report:SustainabilityReport;records:ActivityRecord[];metricSubmissions:MetricSubmission[];targets:ReductionTarget[];indicators:Indicator[];chartType:"bar"|"line"}){
  ...
  if(source==="온실가스 배출량")values=years.map(year=>({label:String(year),value:records.filter(record=>record.status==="확정"&&record.active!==false&&(report.organization==="세원그룹"||record.company===report.organization)&&Number(record.period.slice(0,4))===year).reduce((sum,record)=>sum+record.emissions,0)}));
    ...
    if(indicator)values=years.flatMap(year=>{const value=annualMetricValue(indicator,confirmed,year);return value===null?[]:[{label:String(year),value}];});
  }
...
function MetricAggregationInfo({indicator,rows,years}:{indicator:Indicator;rows:MetricSubmission[];years:number[]}){
  const submitted=rows.filter(row=>row.indicatorId===indicator.id);
  const expected=(year:number)=>indicator.cycle==="월"?monthsForYear(String(year)):indicator.cycle==="분기"?["03","06","09","12"].map(month=>year+"-"+month):indicator.cycle==="반기"?[year+"-06",year+"-12"]:indicator.cycle==="연"?[year+"-12"]:[];
  return <details className="operations-aggregation"><summary>집계 근거 · 포함 조직·기간·추정값 확인</summary><p>확정자료만 포함 · 조직별 기간 {indicator.aggregation??"합계"} 후 조직 {indicator.organizationAggregation??(indicator.unit.includes("%")||indicator.unit.includes("/")?"비율 재계산":"합계")} · 실제 0 포함 / 해당 없음 제외</p>{years.map(year=>{const result=aggregateMetric(indicator,submitted,{year,expectedPeriods:expected(year)});return <div key={year}><strong>{year}년: {result.value===null?"집계 불가 / 자료 없음":formatNumber(result.value,2)+" "+indicator.unit}</strong><p>추정 {result.estimatedCount}건 · 실제 0 {result.zeroCount}건 · 해당 없음 {result.notApplicableCount}건 · 등록된 조직 내 미입력 {result.missingCount}건</p>{result.warnings.map((warning,index)=><p className="form-warning" key={index}>{warning}</p>)}<table><thead><tr><th>포함 법인·사업장</th><th>포함 기준월</th><th>조직별 기간 집계</th></tr></thead><tbody>{result.groups.map(group=><tr key={group.company+"|"+group.site}><td>{group.company} / {group.site||"사업장 미지정"}</td><td>{group.periods.join(", ")||"자료 없음"}</td><td>{group.value===null?"—":formatNumber(group.value,2)}</td></tr>)}</tbody></table></div>})}<p>위 누락 수는 등록된 조직과 지표 수집 주기로 계산합니다. 전체 요청 누락은 사업장별 요청·제출 현황에서 확인하세요.</p></details>;
}
...
function ReportStandards({report,indicators,standards,submissions}:{report:SustainabilityReport;indicators:Indicator[];standards:DisclosureStandard[];submissions:MetricSubmission[]}){
  ...
  const readiness=(id:number)=>{const values=submissions.filter(row=>row.indicatorId===id&&row.period.startsWith(String(report.year))&&(report.organization==="세원그룹"||row.company===report.organization));return {total:values.length,confirmed:values.filter(row=>row.status==="확정").length,pending:values.filter(row=>row.status==="검토대기").length,rejected:values.filter(row=>row.status==="반려").length};};
  ...
  return <section className="card report-standards"><CardHeader title="공시기준 대응표" subtitle="지표 정의서의 기준 연결과 보고연도 제출자료의 실제 준비 상태를 함께 확인합니다. 기준 충족 여부는 담당자가 검토해 주세요."/><div className="standard-summary">{report.frameworks.map(framework=><div key={framework}><span>{framework}</span><strong>{rows.filter(row=>row.standard===framework).length}<small>개 항목</small></strong></div>)}</div><div className="table-scroll"><table className="data-table"><thead><tr><th>기준</th><th>공시 항목</th><th>SEMS 지표</th><th>담당</th><th>보고서 위치</th><th>지표 정의 상태</th><th>보고연도 자료 준비</th></tr></thead><tbody>{rows.map((row,index)=><tr key={`${row.indicator.id}-${row.framework}-${index}`}><td><strong className="standard-code">{row.standard}</strong><span>{standards.find(item=>item.code===row.standard)?.version||"버전 미지정"}</span></td><td>{row.framework}</td><td><strong>{row.indicator.code}</strong><span>{row.indicator.name}</span></td><td>{row.indicator.owner||"미지정"}</td><td>{pageFor(row.indicator)}</td><td><StatusBadge status={row.indicator.status}/></td><td>{(()=>{const data=readiness(row.indicator.id);return <><strong>{!data.total?"자료 없음":data.confirmed===data.total?"등록자료 확정 완료":"추가 검토 필요"}</strong><span>확정 {data.confirmed}/{data.total} · 검토대기 {data.pending} · 보완 {data.rejected}</span></>})()}</td></tr>)}</tbody></table>{!rows.length&&<div className="empty-state"><Icon name="list"/><strong>연결된 공시기준 항목이 없습니다.</strong><p>ESG 지표 정의서의 ‘연결 평가·공시기준’에 GRI, ESRS 또는 KSSB 코드를 등록해 주세요.</p></div>}</div></section>;
}
...
function BulkImport({records,periods,factors,organizations,onClose,onImport}:{records:ActivityRecord[];periods:CollectionPeriod[];factors:EmissionFactor[];organizations:Record<string,string[]>;onClose:()=>void;onImport:(rows:ActivityRecord[])=>void}){
  const availablePeriods=periods.filter(period=>canCollect(period));
  ...
  const mappingErrors=headers.length?validateImportMapping(mapping,headers):[];
  const result=selectedPeriod&&rawRows.length&&!validateImportMapping(mapping,headers).length?validateActivityImport({rows:rawRows,mapping,collectionId:periodId,organizations,tasks:buildGHGCollectionTasks(selectedPeriod),factors,existingRecords:records}):{validRows:[],issues:[],totalRows:rawRows.length,duplicateCount:0};
  const failedRows=[...new Set(result.issues.map(issue=>issue.rowNumber))];
  const page=Math.min(issuePage,Math.max(0,Math.ceil(result.issues.length/50)-1));
  const downloadTemplate=async()=>{const XLSX=await import("xlsx");const task=selectedPeriod?buildGHGCollectionTasks(selectedPeriod)[0]:undefined;const factor=factors.find(item=>item.scope===task?.targetId&&isFactorEffective(item,task?.period??""));const company=task?.company??Object.keys(organizations)[0]??"";const templateHeaders=IMPORT_FIELDS.filter(field=>field.key!=="evidence").map(field=>field.label);const sample:Record<string,string|number>={company,site:task?.site??organizations[company]?.[0]??"",period:task?.period??"",scope:task?.targetId??"Scope 1",category:factor?.category??"",source:factor?.source??"",usage:1000,unit:factor?.activityUnit??"",dataStatus:"actual",owner:"담당자명",department:"담당 부서",description:"산정 기준과 변동 사유"};const sheet=XLSX.utils.aoa_to_sheet([templateHeaders,IMPORT_FIELDS.filter(field=>field.key!=="evidence").map(field=>sample[field.key]??"")]);sheet["!cols"]=templateHeaders.map(()=>({wch:20}));const guide=XLSX.utils.aoa_to_sheet([["SEMS 활동자료 가져오기"],["자료 구분: actual 실측값, zero 실제 0, not_applicable 해당 없음, estimated 추정값"],["해당 없음과 추정값은 입력 설명이 필요합니다. 빈 사용량은 실제 0으로 간주하지 않습니다."],["같은 수집기간·사업장·기준월·배출원은 중복 등록되지 않습니다."],["열 제목이 달라도 가져오기 화면에서 연결할 수 있습니다. 선택한 시트의 첫 행을 제목으로 사용합니다."],["계수의 적용기간과 활동량 단위를 검증하고 산정 당시 계수를 보존합니다."]]);const book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,sheet,"활동자료 입력");XLSX.utils.book_append_sheet(book,guide,"작성 안내");XLSX.writeFile(book,"SEMS_activity_import_template.xlsx");};
  const readSheet=async(book:import("xlsx").WorkBook,name:string)=>{setSheetName(name);setRawRows([]);setHeaders([]);setMapping({});setIssuePage(0);setError("");const XLSX=await import("xlsx");const sheet=book.Sheets[name];if(!sheet)throw Error("선택한 시트를 찾을 수 없습니다.");const values=XLSX.utils.sheet_to_json<unknown[]>(sheet,{header:1,defval:"",blankrows:true});const columns=(values[0]??[]).map(value=>String(value??"").trim());if(!columns.some(Boolean))throw Error("선택한 시트의 첫 행에 열 제목이 필요합니다.");setHeaders(columns);setMapping(suggestColumnMapping(columns));setRawRows(values.slice(1).map(row=>Object.fromEntries(columns.map((header,index)=>[header,row[index]??""]))));};
  const changeSheet=async(name:string)=>{if(!workbook)return;setLoading(true);try{await readSheet(workbook,name);}catch(issue){setError(issue instanceof Error?issue.message:"시트를 읽을 수 없습니다.");}finally{setLoading(false);}};
  const readFile=async(event:ChangeEvent<HTMLInputElement>)=>{const file=event.target.files?.[0];if(!file)return;setError("");setLoading(true);setRawRows([]);setHeaders([]);setWorkbook(null);try{const XLSX=await import("xlsx");const book=XLSX.read(await file.arrayBuffer(),{type:"array"});if(!book.SheetNames.length)throw Error("빈 파일입니다.");setWorkbook(book);setFileName(file.name);await readSheet(book,book.SheetNames[0]);}catch(issue){setError(issue instanceof Error?issue.message:"파일을 읽을 수 없습니다. Excel 또는 CSV 파일을 확인해 주세요.");}finally{setLoading(false);event.target.value="";}};
  const exportIssues=()=>{downloadCsv("SEMS_import_errors.csv",[...headers,"오류 행","확인할 내용"],failedRows.map(rowNumber=>[...headers.map(header=>String(rawRows[rowNumber-2]?.[header]??"")),rowNumber,result.issues.filter(issue=>issue.rowNumber===rowNumber).map(issue=>issue.message).join(" / ")]));};
  const commit=()=>{if(!selectedPeriod||!canCollect(selectedPeriod)||mappingErrors.length)return;const latest=validateActivityImport({rows:rawRows,mapping,collectionId:periodId,organizations,tasks:buildGHGCollectionTasks(selectedPeriod),factors,existingRecords:records});if(!latest.validRows.length){setError("등록할 행이 없습니다. 중복과 오류를 확인해 주세요.");return;}const nextId=records.reduce((max,row)=>Math.max(max,row.id),0)+1;const at=nowLabel();onImport(latest.validRows.map(({rowNumber,...row})=>({...row,id:nextId+rowNumber-2,collectionId:periodId,status:"작성중",active:true,createdAt:at,updatedAt:at})));};
  return <Overlay title="Excel·CSV 일괄등록" eyebrow="BULK IMPORT" description="열을 연결하고 모든 오류를 검토한 뒤, 검증을 통과한 행을 등록합니다." onClose={onClose}><div className="form-section"><div className="form-grid"><label>수집기간<select value={periodId} onChange={event=>{setPeriodId(event.target.value);setIssuePage(0)}}>{availablePeriods.map(period=><option value={period.id} key={period.id}>{period.name}</option>)}</select></label><div className="bulk-buttons"><button type="button" className="secondary-button" onClick={downloadTemplate} disabled={!selectedPeriod}>양식 다운로드</button><button type="button" className="primary-button" onClick={()=>inputRef.current?.click()} disabled={!selectedPeriod||loading}>{loading?"읽는 중…":fileName?"수정 파일 재업로드":"파일 선택"}</button><input ref={inputRef} type="file" accept=".xlsx,.xls,.csv" hidden onChange={readFile}/></div></div>{workbook&&<label className="operations-sheet-picker">가져올 시트<select value={sheetName} disabled={loading} onChange={event=>{void changeSheet(event.target.value)}}>{workbook.SheetNames.map(name=><option key={name}>{name}</option>)}</select><small>시트를 바꾸면 열 연결과 검증 결과를 새로 확인합니다.</small></label>}{fileName&&<p className="operations-help">{fileName} · {sheetName} · 데이터 {result.totalRows}행</p>}{!selectedPeriod&&<p className="form-warning">현재 입력 가능한 수집기간이 없습니다.</p>}{error&&<p role="alert" className="form-error">{error}</p>}</div>
    {headers.length>0&&<div className="form-section"><h3><span>1</span>열 연결</h3><p className="operations-help">필수 항목의 열을 확인하세요. 기존 SEMS 양식은 자동으로 연결됩니다.</p><div className="operations-import-mapping">{IMPORT_FIELDS.filter(field=>field.key!=="evidence"||mapping.evidence).map(field=><label key={field.key}>{field.label}{field.required?" *":" (선택)"}<select value={mapping[field.key]??""} onChange={event=>{setMapping(current=>({...current,[field.key]:event.target.value||undefined}));setIssuePage(0)}}><option value="">연결 안 함</option>{headers.map((header,index)=><option key={index} value={header}>{header||"(제목 없음)"}</option>)}</select></label>)}</div>{mappingErrors.map((message,index)=><p className="form-error" key={index}>{message}</p>)}</div>}
    {rawRows.length>0&&!mappingErrors.length&&<div className="form-section"><h3><span>2</span>검증 결과</h3><div className="operations-summary"><div><span>등록 가능</span><strong>{result.validRows.length}<small>행</small></strong></div><div><span>확인할 자료</span><strong>{failedRows.length}<small>행</small></strong></div><div><span>중복 제외</span><strong>{result.duplicateCount}<small>행</small></strong></div></div>{result.issues.length>0&&<><div className="operations-section-title"><strong>오류 {result.issues.length}건 · 모든 오류를 페이지별로 확인할 수 있습니다.</strong><button type="button" className="secondary-button" onClick={exportIssues}>오류 행 CSV 다운로드</button></div><div className="table-scroll"><table className="data-table"><thead><tr><th>원본 행</th><th>항목</th><th>입력값</th><th>확인할 내용</th></tr></thead><tbody>{result.issues.slice(page*50,(page+1)*50).map((issue,index)=><tr key={index}><td>{issue.rowNumber}</td><td>{IMPORT_FIELDS.find(field=>field.key===issue.field)?.label??"자료"}</td><td>{issue.rawValue||"(빈 값)"}</td><td>{issue.message}</td></tr>)}</tbody></table></div><div className="operations-pagination"><button type="button" className="secondary-button" disabled={page===0} onClick={()=>setIssuePage(page-1)}>이전</button><span>{page+1} / {Math.max(1,Math.ceil(result.issues.length/50))}</span><button type="button" className="secondary-button" disabled={(page+1)*50>=result.issues.length} onClick={()=>setIssuePage(page+1)}>다음</button></div></>}{result.validRows.length>0&&<div className="table-scroll"><table className="data-table"><thead><tr><th>원본 행</th><th>법인 / 사업장</th><th>기준월</th><th>배출원</th><th>자료 구분</th><th>사용량</th><th>배출량(tCO₂e)</th></tr></thead><tbody>{result.validRows.slice(0,20).map(row=><tr key={row.rowNumber}><td>{row.rowNumber}</td><td>{row.company} / {row.site}</td><td>{row.period}</td><td>{row.source}</td><td>{DATA_STATUS_LABELS[row.dataStatus]}</td><td>{formatNumber(row.usage,3)} {row.unit}</td><td>{formatNumber(row.emissions,4)}</td></tr>)}</tbody></table>{result.validRows.length>20&&<p className="operations-help">등록 가능 행 중 처음 20개를 미리 표시합니다. 총 {result.validRows.length}개 행이 등록됩니다.</p>}</div>}<p className="operations-help">오류 행을 수정해 다시 올려도 이미 등록된 같은 자료는 중복으로 제외됩니다. 정상 행은 작성중 상태로 저장됩니다.</p></div>}
    <div className="modal-footer"><button type="button" className="secondary-button" onClick={onClose}>취소</button><button type="button" className="primary-button" disabled={!result.validRows.length||!!mappingErrors.length||loading} onClick={commit}>검증 통과 {result.validRows.length}행 등록</button></div></Overlay>;
}

function Settings({factors,records,onFactorsChange,criteria,onCriteriaChange,noticePrefs,onNoticePrefsChange,organizations,onOrganizationsChange,onExport,onRestore,showToast}:{factors:EmissionFactor[];records:ActivityRecord[];onFactorsChange:(x:EmissionFactor[])=>void;criteria:CollectionCriteria;onCriteriaChange:(x:CollectionCriteria)=>void;noticePrefs:NotificationPrefs;onNoticePrefsChange:(x:NotificationPrefs)=>void;organizations:Record<string,string[]>;onOrganizationsChange:(x:Record<string,string[]>)=>void;onExport:()=>void;onRestore:(payload:Record<string,unknown>)=>void;showToast:(m:string)=>void}){
  const [tab,setTab]=useState<SettingTab>("factors"); const [factorModal,setFactorModal]=useState<EmissionFactor|null|"new">(null);
  const saveFactor=(factor:EmissionFactor)=>{const normalized=factor.id==="NEW-FACTOR"?{...factor,id:`F-${crypto.randomUUID()}`}:factor;const exists=factors.some(f=>f.id===normalized.id);onFactorsChange(exists?factors.map(f=>f.id===normalized.id?normalized:f):[...factors,normalized]);setFactorModal(null);showToast(exists?"배출계수를 수정했습니다.":"새 배출계수를 추가했습니다.");};
  ...
  return <><PageHeader eyebrow="SYSTEM SETTINGS" title="시스템 설정" description="조직, 산정 기준, 알림과 운영 데이터 백업을 관리합니다."/><div className="settings-layout"><aside className="settings-nav"><button className={tab==="organization"?"active":""} onClick={()=>setTab("organization")}><Icon name="building" size={18}/>조직·사업장</button><button className={tab==="factors"?"active":""} onClick={()=>setTab("factors")}><Icon name="leaf" size={18}/>배출계수·산정기준</button><button className={tab==="criteria"?"active":""} onClick={()=>setTab("criteria")}><Icon name="list" size={18}/>수집 기준</button><button className={tab==="notifications"?"active":""} onClick={()=>setTab("notifications")}><Icon name="bell" size={18}/>알림 설정</button><button className={tab==="data"?"active":""} onClick={()=>setTab("data")}><Icon name="database" size={18}/>데이터 백업</button></aside><section className="card settings-content">
    ...
    {tab==="data"&&<DataSettings onExport={onExport} onRestore={onRestore} showToast={showToast}/>}</section></div>{factorModal&&<FactorForm factor={factorModal==="new"?null:factorModal} records={records} onClose={()=>setFactorModal(null)} onSave={saveFactor} onDelete={factorModal==="new"?undefined:()=>removeFactor(factorModal.id)}/>}</>;
}
/*
...
function FactorForm({factor,records=[],onClose,onSave,onDelete}:{factor:EmissionFactor|null;records?:ActivityRecord[];onClose:()=>void;onSave:(f:EmissionFactor)=>void;onDelete?:()=>void}){
  ...
  const affected=records.filter(row=>row.active!==false&&(row.factorId===factor?.id||!row.factorId&&row.scope===factor?.scope&&row.category===factor?.category&&row.source===factor?.source));
  const impact=affected.map(row=>({row,calculation:calculateEmission({usage:row.usage,unit:row.unit,period:row.period,factor:form})}));
  const impactDelta=impact.reduce((sum,item)=>sum+(item.calculation.ok?item.calculation.emissions-item.row.emissions:0),0);
...
function BatchRecordEntry({records,periods,factors,organizations,defaultOwner,defaultDepartment,onClose,onSave,showToast}:{records:ActivityRecord[];periods:CollectionPeriod[];factors:EmissionFactor[];organizations:Record<string,string[]>;defaultOwner:string;defaultDepartment:string;onClose:()=>void;onSave:(rows:ActivityRecord[])=>void;showToast:(message:string)=>void}){
  const review=useSubmissionReview<ActivityRecord[]>(onSave);
  ...
  const [rows,setRows]=useState<ActivityInputRow[]>([{key:"input-row-1",factorId:initialFactor?.id??"",usage:0,dataStatus:"missing",evidence:"",description:""}]);
  const [error,setError]=useState("");
  ...
  const availableFactors=factors.filter(factor=>factor.scope===context.scope&&isFactorEffective(factor,context.period));
  ...
  const resetRowFactors=(scope:Scope)=>{const first=factors.find(factor=>factor.active&&factor.scope===scope);setRows(current=>current.map((row,index)=>({...row,factorId:index===0?(first?.id??""):"",usage:0,dataStatus:"missing"})));};
  ...
  const addRow=()=>{const used=new Set(rows.map(row=>row.factorId));const nextFactor=availableFactors.find(factor=>!used.has(factor.id))??availableFactors[0];setRows(current=>[...current,{key:`input-row-${nextKey.current++}`,factorId:nextFactor?.id??"",usage:0,dataStatus:"missing",evidence:"",description:""}]);setError("");};
  ...
  const duplicateRow=(row:ActivityInputRow)=>setRows(current=>[...current,{...row,key:`input-row-${nextKey.current++}`,usage:0,dataStatus:"missing",evidence:""}]);
  const removeRow=(key:string)=>setRows(current=>current.length===1?[{...current[0],factorId:availableFactors[0]?.id??"",usage:0,dataStatus:"missing",evidence:"",description:""}]:current.filter(row=>row.key!==key));
  ...
  const rowEmission=(row:ActivityInputRow)=>{const factor=factorFor(row);const calculation=factor?calculateEmission({usage:row.usage,unit:factor.activityUnit,period:context.period,factor}):null;return calculation?.ok?calculation.emissions:0;};
  const totalEmissions=rows.reduce((sum,row)=>sum+rowEmission(row),0);
  const exportRows=()=>{downloadCsv(`SEMS_${context.scope.replace(" ","")}_${context.period}_입력표.csv`,["활동자료 구분","배출원","사용량","단위","배출계수","예상 배출량(tCO2e)","자료 구분","증빙","설명"],rows.map(row=>{const factor=factorFor(row);return [factor?.category??"",factor?.source??"",row.usage,factor?.activityUnit??"",factor?.value??"",factor?rowEmission(row):"",row.dataStatus??(row.usage===0?"zero":"actual"),row.evidence,row.description]}));showToast("현재 입력표를 CSV로 내려받았습니다.");};
  const importRows=async(event:ChangeEvent<HTMLInputElement>)=>{const file=event.target.files?.[0];if(!file||!selectedPeriod)return;try{const XLSX=await import("xlsx");const book=XLSX.read(await file.arrayBuffer(),{type:"array"});const sheet=book.Sheets[book.SheetNames[0]];const values=XLSX.utils.sheet_to_json<Record<string,unknown>>(sheet,{defval:""});const prepared=values.map(value=>({...value,"법인":context.company,"사업장":context.site,"자료 기준월":context.period,"Scope":context.scope,"활동자료 구분":value["활동자료 구분"]??value["구분"]??"","담당자":context.owner,"담당 부서":context.department,"값 구분":value["자료 구분"]??value["값 구분"]??"","입력 설명":value["설명"]??value["입력 설명"]??"","증빙 파일명":value["증빙"]??value["증빙 파일명"]??""}));const mapping=suggestColumnMapping(Object.keys(prepared[0]??{}));mapping.dataStatus="값 구분";mapping.description="입력 설명";mapping.evidence="증빙 파일명";const staged=rows.filter(row=>row.dataStatus!=="missing").flatMap(row=>{const factor=factorFor(row);return factor?[{...context,category:factor.category,source:factor.source}]:[];});const checked=validateActivityImport({rows:prepared,mapping,collectionId:context.collectionId,organizations,tasks:buildGHGCollectionTasks(selectedPeriod),factors:availableFactors,existingRecords:[...records,...staged]});const imported=checked.validRows.map(row=>({key:"input-row-"+nextKey.current++,factorId:row.factorId,usage:row.usage,dataStatus:row.dataStatus,evidence:row.evidence,description:row.description}));if(!imported.length){setError(checked.issues.map(issue=>issue.rowNumber+"행: "+issue.message).join(" / ")||"불러올 수 있는 행이 없습니다.");return;}setRows(current=>current.length===1&&current[0].dataStatus==="missing"?imported:[...current,...imported]);setError(checked.issues.map(issue=>issue.rowNumber+"행: "+issue.message).join(" / "));showToast(imported.length+"개 행을 입력표에 불러왔습니다.");}catch{setError("파일을 읽을 수 없습니다. 현재 입력표에서 내보낸 양식을 사용해 주세요.");}finally{event.target.value="";}};
  const submit=(event:FormEvent)=>{event.preventDefault();if(!selectedPeriod||!selectedTasks.some(task=>task.company===context.company&&(!task.site||task.site===context.site)&&task.targetId===context.scope&&task.period===context.period)){setError("현재 수집 요청의 회사·사업장·Scope·기준월을 선택해 주세요.");return;}if(!context.site){setError("사업장을 선택해 주세요.");return;}if(!rows.length){setError("입력행을 추가해 주세요.");return;}if(duplicateIdentities.size||rows.some(row=>existingIdentities.has(rowIdentity(row)))){setError("같은 배출원이 입력표 또는 기존 자료에 중복되어 있습니다.");return;}if(!context.owner.trim()||!context.department.trim()){setError("공통 담당자와 담당 부서를 입력해 주세요.");return;}const result:ActivityRecord[]=[];for(const [index,row] of rows.entries()){const error=validateDataValue({value:row.usage,dataStatus:row.dataStatus,description:row.description});const factor=factorFor(row);if(error||!factor){setError((index+1)+"행: "+(error||"배출계수를 선택해 주세요."));return;}const calculation=calculateEmission({usage:row.usage,unit:factor.activityUnit,period:context.period,factor});if(!calculation.ok){setError((index+1)+"행: "+calculation.errors.join(" "));return;}result.push({id:0,collectionId:context.collectionId,company:context.company,site:context.site,period:context.period,scope:context.scope,category:factor.category,source:factor.source,usage:row.usage,dataStatus:dataValueStatus({value:row.usage,dataStatus:row.dataStatus}),unit:factor.activityUnit,factor:factor.value,factorId:factor.id,factorSnapshot:calculation.snapshot,emissions:calculation.emissions,owner:context.owner.trim(),department:context.department.trim(),status:submissionIntent(event),evidence:row.evidence,description:row.description,active:true,updatedAt:"방금 전"});}review.save(result,submissionIntent(event));};
  return <Overlay title="활동자료 일괄 입력" eyebrow="SEMS INPUT SHEET" description="회사·사업장·연도·월을 한 번 선택하고 여러 배출원을 행으로 추가해 함께 저장합니다." onClose={onClose} size="wide"><GuidedForm onSubmit={submit} className="batch-entry-form">
    <div className="batch-context-bar"><label>수집기간<SimpleSelect value={context.collectionId} onChange={event=>changeCollection(event.target.value)}>{editablePeriods.map(period=><option value={period.id} key={period.id}>{period.name}</option>)}</SimpleSelect></label><label>회사<SimpleSelect value={context.company} onChange={event=>changeCompany(event.target.value)}>{companies.map(company=><option key={company}>{company}</option>)}</SimpleSelect></label><label>사업장<SimpleSelect value={context.site} onChange={event=>setContext(current=>({...current,site:event.target.value}))}>{(organizations[context.company]??[]).map(site=><option key={site}>{site}</option>)}</SimpleSelect></label><label>연도<SimpleSelect value={selectedYear} onChange={event=>changeYear(event.target.value)}>{years.map(year=><option key={year}>{year}년</option>)}</SimpleSelect></label><label>월<SimpleSelect value={context.period} onChange={event=>setContext(current=>({...current,period:event.target.value}))}>{visibleMonths.map(month=><option value={month} key={month}>{Number(month.slice(5))}월</option>)}</SimpleSelect></label><label>Scope<SimpleSelect value={context.scope} onChange={event=>changeScope(event.target.value as Scope)}>{scopes.map(scope=><option key={scope}>{scope}</option>)}</SimpleSelect></label></div>
    ...
    <div className="batch-table-scroll"><table className="batch-entry-table"><thead><tr><th>번호</th><th>활동자료 구분</th><th>배출원</th><th>자료 구분</th><th>사용량</th><th>단위</th><th>배출계수</th><th>예상 배출량</th><th>증빙</th><th>입력 설명</th><th>작업</th></tr></thead><tbody>{rows.map((row,index)=>{const factor=factorFor(row);const sources=availableFactors.filter(item=>item.category===factor?.category);const identity=rowIdentity(row);const duplicated=duplicateIdentities.has(identity)||existingIdentities.has(identity);return <tr key={row.key} className={duplicated?"batch-row-error":""}><td><span className="batch-row-number">{index+1}</span></td><td><SimpleSelect value={factor?.category??""} onChange={event=>chooseCategory(row,event.target.value)} aria-label={`${index+1}행 활동자료 구분`}><option value="">구분 선택</option>{categories.map(category=><option key={category}>{category}</option>)}</SimpleSelect></td><td><SimpleSelect value={row.factorId} onChange={event=>patchRow(row.key,{factorId:event.target.value})} aria-label={`${index+1}행 배출원`}><option value="">배출원 선택</option>{sources.map(source=><option value={source.id} key={source.id}>{source.source}</option>)}</SimpleSelect>{duplicated&&<small>중복 배출원</small>}</td><td><select aria-label={(index+1)+"행 자료 구분"} value={row.dataStatus??"actual"} onChange={event=>patchRow(row.key,{dataStatus:event.target.value as DataStatus,...(["zero","not_applicable"].includes(event.target.value)?{usage:0}:{})})}><option value="missing">미입력</option><option value="actual">실측값</option><option value="zero">실측 0</option><option value="not_applicable">해당 없음</option><option value="estimated">추정값</option></select></td><td><input type="number" min="0" required step="any" value={row.dataStatus==="missing"?"":row.usage} disabled={row.dataStatus==="zero"||row.dataStatus==="not_applicable"} onChange={event=>patchRow(row.key,{usage:Number(event.target.value),dataStatus:event.target.value===""?"missing":row.dataStatus==="estimated"?"estimated":"actual"})} placeholder="0" aria-label={`${index+1}행 사용량`}/></td><td className="batch-readonly">{factor?.activityUnit??"-"}</td><td className="batch-readonly mono">{factor?formatNumber(factor.value,factor.value<10?5:1):"-"}</td><td className="batch-emissions"><strong>{factor?formatNumber(rowEmission(row),2):"0.00"}</strong><span>tCO₂e</span></td><td><label className="batch-file-button"><input type="file" accept=".pdf,.xlsx,.xls,.jpg,.jpeg,.png" onChange={event=>{const file=event.target.files?.[0];if(file)patchRow(row.key,{evidence:file.name})}}/><Icon name={row.evidence?"file":"upload"} size={14}/><span title={row.evidence}>{row.evidence||"파일"}</span></label></td><td><input value={row.description} onChange={event=>patchRow(row.key,{description:event.target.value})} placeholder="산정 근거·변동 사유" aria-label={`${index+1}행 입력 설명`}/></td><td><div className="batch-row-actions"><button type="button" onClick={()=>duplicateRow(row)} aria-label={`${index+1}행 복제`} title="행 복제"><Icon name="copy" size={14}/></button><button type="button" className="danger" onClick={()=>removeRow(row.key)} aria-label={`${index+1}행 삭제`} title="행 삭제"><Icon name="trash" size={14}/></button></div></td></tr>})}</tbody></table></div>
    ...
    <div className="modal-footer"><button type="button" className="secondary-button" onClick={onClose}>취소</button><SubmissionButtons disabled={!availableFactors.length}/></div>
  </GuidedForm>{review.dialog}</Overlay>;
}
...
function RecordModal({record,records,periods,factors,criteria,organizations,defaultOwner,defaultDepartment,onClose,onSave}:{record:ActivityRecord|null;records:ActivityRecord[];periods:CollectionPeriod[];factors:EmissionFactor[];criteria:CollectionCriteria;organizations:Record<string,string[]>;defaultOwner:string;defaultDepartment:string;onClose:()=>void;onSave:(r:ActivityRecord)=>void}){
  ...
  const [form,setForm]=useState<ActivityRecord>(record??{id:0,collectionId:defaultPeriod?.id,company:defaultCompany,site:organizations[defaultCompany]?.[0]??"",period:defaultTask?.period??defaultPeriod?.dataFrom??new Date().toISOString().slice(0,7),dataStatus:"missing",factorId:fallback.id,scope:fallback.scope,category:fallback.category,source:fallback.source,usage:0,unit:fallback.activityUnit,factor:fallback.value,emissions:0,owner:defaultOwner,department:defaultDepartment,status:"작성중",evidence:"",description:"",active:true,updatedAt:"방금 전"});
  const [error,setError]=useState("");
  ...
  const available=factors.filter(f=>f.scope===form.scope&&isFactorEffective(f,form.period)); const categories=[...new Set(available.map(f=>f.category))]; const sources=available.filter(f=>f.category===form.category); const selected=available.find(f=>f.id===form.factorId&&f.category===form.category&&f.source===form.source)??available.find(f=>f.scope===form.scope&&f.category===form.category&&f.source===form.source)??null; const patch=(p:Partial<ActivityRecord>)=>setForm(c=>({...c,...p}));
  const applyFactor=(f:EmissionFactor)=>patch({factorId:f.id,category:f.category,source:f.source,unit:f.activityUnit,factor:f.value});
  ...
  const duplicate=records.some(item=>item.active!==false&&item.id!==form.id&&item.collectionId===form.collectionId&&item.company===form.company&&item.site===form.site&&item.period===form.period&&item.scope===form.scope&&item.category===form.category&&item.source===form.source);
  ...
  const activityChanged=!record||record.id===0||["usage","unit","period","scope","category","source","factorId"].some(key=>form[key as keyof ActivityRecord]!==record[key as keyof ActivityRecord]);
  const calculationFactor=activityChanged?selected:record?.factorSnapshot?{...record.factorSnapshot,active:true}:null;
  const calculation=calculationFactor?calculateEmission({usage:form.usage,unit:form.unit,period:form.period,factor:calculationFactor}):null;
  const submit=(e:FormEvent)=>{e.preventDefault();if(!canCollect(selectedPeriod)){setError("현재 수집중인 기간을 선택해 주세요.");return;}if(!selectedTasks.some(task=>task.company===form.company&&(!task.site||task.site===form.site)&&task.targetId===form.scope&&task.period===form.period)){setError("이 요청에 생성된 세부 수집 항목을 선택해 주세요.");return;}const validation=validateDataValue({value:form.usage,dataStatus:form.dataStatus,description:form.description});if(validation||form.usage<0){setError(validation||"사용량은 0 이상이어야 합니다.");return;}if(duplicate){setError("같은 사업장·자료 기준월·활동자료가 이미 등록되어 있습니다.");return;}if(activityChanged&&(!calculation||!calculation.ok)){setError(calculation&&!calculation.ok?calculation.errors.join(" "):"현재 기간에 적용할 배출계수를 선택해 주세요.");return;}const computed=activityChanged&&calculation?.ok?{factorId:calculation.snapshot.id,factorSnapshot:calculation.snapshot,factor:calculation.snapshot.value,emissions:calculation.emissions}:{};review.save({...form,...computed,dataStatus:dataValueStatus({value:form.usage,dataStatus:form.dataStatus}),status:submissionIntent(e),rejectionReason:submissionIntent(e)==="검토대기"?"":form.rejectionReason,updatedAt:nowLabel()},submissionIntent(e));};
  ...
  return <Overlay title={record?"활동자료 수정":"신규 활동자료 입력"} eyebrow="ACTIVITY DATA" description="수집기간과 Scope에 맞는 활동자료·배출계수가 자동 연결됩니다." onClose={onClose}><GuidedForm onSubmit={submit}>
    <div className="form-section"><h3><span>1</span>수집기간 및 기본 정보</h3><div className="form-grid"><label className="full-span">수집기간<SimpleSelect value={form.collectionId} onChange={e=>changePeriod(e.target.value)}>{editablePeriods.map(period=><option value={period.id} key={period.id}>{period.name} · {period.status}</option>)}</SimpleSelect></label><label>법인<SimpleSelect value={form.company} onChange={e=>changeCompany(e.target.value)}>{availableCompanies.map(c=><option key={c}>{c}</option>)}</SimpleSelect></label><label>사업장<SimpleSelect value={form.site} onChange={e=>patch({site:e.target.value})}>{(organizations[form.company]??[]).map(s=><option key={s}>{s}</option>)}</SimpleSelect></label><label>자료 기준월<SimpleSelect value={form.period} onChange={e=>patch({period:e.target.value})}>{availableMonths.map(month=><option key={month}>{month}</option>)}</SimpleSelect></label><label>Scope<SimpleSelect value={form.scope} onChange={e=>changeScope(e.target.value as Scope)}>{availableScopes.map(scope=><option key={scope}>{scope}</option>)}</SimpleSelect></label></div></div>
    ...
    <div className="form-section"><h3><span>2</span>활동자료 및 산정</h3><DataStatusFields value={form.usage} status={form.dataStatus} description={form.description} onChange={dataStatus=>patch({dataStatus,...(["zero","not_applicable"].includes(dataStatus)?{usage:0}:{})})}/>{available.length?<><div className="form-grid"><label>활동자료 구분<SimpleSelect value={form.category} onChange={e=>{const first=available.find(f=>f.category===e.target.value);if(first)applyFactor(first)}}>{categories.map(c=><option key={c}>{c}</option>)}</SimpleSelect></label><label>배출원<SimpleSelect value={selected?.id??""} onChange={e=>{const found=sources.find(f=>f.id===e.target.value);if(found)applyFactor(found)}}><option value="">배출계수 선택</option>{sources.map(f=><option key={f.id} value={f.id}>{f.source} · {f.year}</option>)}</SimpleSelect></label><label>사용량<div className="input-unit"><input type="number" min="0" step="any" value={form.dataStatus==="missing"?"":form.usage} disabled={form.dataStatus==="not_applicable"||form.dataStatus==="zero"} onChange={e=>patch({usage:Number(e.target.value),dataStatus:e.target.value===""?"missing":form.dataStatus==="estimated"?"estimated":"actual"})} required/><span>{form.unit}</span></div></label><label>단위<input value={form.unit} readOnly className="readonly-input"/></label><label>배출계수<div className="locked-input"><input value={activityChanged&&selected?selected.value:form.factor} readOnly tabIndex={-1}/><Icon name="lock" size={15}/></div><small className="field-help">{selected?.factorType??"운영계수"} · {selected?.year} · {selected?.authority}<br/>{selected?.method??"활동량 × 배출계수"} / 시스템 설정에서만 변경 가능</small></label><div className="calculated-field"><span>예상 배출량</span><strong>{formatNumber(calculation?.ok?calculation.emissions:form.emissions,2)} <small>tCO₂e</small></strong><em>{calculation?.ok?calculation.formula:"기존 저장값 유지"}</em></div></div>{selected?.notes&&<div className="factor-notice compact-notice"><Icon name="alert" size={16}/><p><strong>계수 적용 기준</strong><br/>{selected.notes}</p></div>}<div className="comparison-grid form-comparison"><ComparisonCard label="전월" record={previous} current={{...form,emissions:0}} threshold={criteria.variance}/><ComparisonCard label="전년 동월" record={previousYear} current={{...form,emissions:0}} threshold={criteria.variance}/></div>{variance!==null&&Math.abs(variance)>=criteria.variance&&<p className="form-warning"><Icon name="alert" size={15}/>전월 대비 {formatNumber(Math.abs(variance),1)}% {variance>0?"증가":"감소"}했습니다. 입력 설명에 변동 사유를 남겨 주세요.</p>}{duplicate&&<p className="form-error"><Icon name="alert" size={14}/>같은 조건의 활동자료가 이미 등록되어 있습니다.</p>}</>:<div className="empty-state"><Icon name="alert"/><strong>이 Scope에 사용 가능한 배출계수가 없습니다.</strong><p>시스템 설정 &gt; 배출계수·산정기준에서 계수를 먼저 등록해 주세요.</p></div>}</div>
    ...
    <div className="modal-footer"><button type="button" className="secondary-button" onClick={onClose}>취소</button><SubmissionButtons rejected={record?.status==="반려"} disabled={(!available.length&&!record)||duplicate}/></div></GuidedForm>{review.dialog}</Overlay>;
}

type MetricEntryContext={request:MetricRequest;indicator:Indicator;company:string;site?:string;period:string;submission?:MetricSubmission};
function submissionIntent(event:FormEvent):RecordStatus {
  return (event.nativeEvent as SubmitEvent).submitter?.getAttribute("value")==="submit"?"검토대기":"작성중";
}
function SubmissionButtons({rejected=false,disabled=false}:{rejected?:boolean;disabled?:boolean}) {
  return <><button type="submit" name="intent" value="draft" className="secondary-button" disabled={disabled}>임시저장</button><button type="submit" name="intent" value="submit" className="primary-button" disabled={disabled}>{rejected?"수정 후 재제출":"내용 확인 후 제출"}</button></>;
}
function GuidedForm({children,onSubmit,className}:{children:ReactNode;onSubmit:(event:FormEvent)=>void;className?:string}) {
  const [issues,setIssues]=useState<{label:string;field:HTMLElement}[]>([]);
  return <form noValidate className={className} onSubmit={event=>{
    const controls=Array.from(event.currentTarget.elements).filter((field):field is HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement=>field instanceof HTMLInputElement||field instanceof HTMLSelectElement||field instanceof HTMLTextAreaElement);
    const invalid=controls.filter(field=>!field.disabled&&!field.validity.valid).map(field=>({field,label:(field.getAttribute("aria-label")||field.closest("label")?.textContent?.slice(0,45)||"입력 항목")+": "+field.validationMessage}));
    setIssues(invalid);if(invalid.length){event.preventDefault();invalid[0].field.focus();return;}onSubmit(event);
  }}>{issues.length>0&&<div className="submission-check" role="alert"><strong>다음 {issues.length}개 항목을 확인해 주세요</strong>{issues.map((issue,index)=><button type="button" key={index} onClick={()=>{issue.field.scrollIntoView({block:"center",behavior:"smooth"});issue.field.focus();}}>{issue.label} →</button>)}</div>}{children}</form>;
}
function useSubmissionReview<T extends {company:string;period:string;unit:string;status:RecordStatus}|ActivityRecord[]>(onSave:(value:T)=>void) {
  const [pending,setPending]=useState<T|null>(null);
  const submitting=useRef(false);
  const [warnings,setWarnings]=useState<string[]>([]);
  const save=(value:T,intent:RecordStatus)=>{
    if(intent!=="검토대기"){onSave(value);return;}
    setWarnings(Array.from(document.querySelectorAll(".overlay .form-warning, .modal-overlay .form-warning, .form-warning")).map(node=>node.textContent??"").filter(Boolean));
    submitting.current=false;setPending(value);
  };
  const rows=pending?(Array.isArray(pending)?pending:[pending]):[];
  const dialog=pending?<Overlay title="제출 전 확인" eyebrow="CHECK & SUBMIT" description="자료 기준과 단위를 확인해 주세요. 제출 후에는 검토가 진행됩니다." onClose={()=>setPending(null)}><div className="submission-check"><p>아직 제출되지 않았습니다. 아래 내용을 확인한 뒤 제출 완료를 눌러 주세요.</p>{rows.map((row,index)=><article key={index}><strong>{row.company} · {row.period}</strong><p>{"source" in row?String(row.source):"ESG 제출값"}{"detailRows" in row&&Array.isArray(row.detailRows)&&row.detailRows.some((detail:MetricDetailRow)=>detail.values.detailKey)?" · 아래 세부값 확인":` · ${"usage" in row?String(row.usage):"value" in row?String(row.value):""} ${row.unit}`}</p>{"detailRows" in row&&Array.isArray(row.detailRows)&&<div><p>상세 입력 {row.detailRows.length}개 행 포함</p>{row.detailRows.map((detail:MetricDetailRow,index:number)=><p key={detail.id}>{index+1}. {String(detail.values.label??"세부 항목")} · {Object.entries(detail.values).filter(([key])=>!["detailKey","label"].includes(key)).map(([key,value])=>`${Object.values(METRIC_TEMPLATE_FIELDS).flat().find(field=>field.key===key)?.label??(key==="unit"?"단위":key)}: ${value}`).join(" / ")}</p>)}</div>}</article>)}{warnings.map((warning,index)=><p className="form-warning" key={index}>{warning}</p>)}<p>증빙은 선택사항입니다. 입력값의 단위와 산정 근거를 확인해 주세요.</p></div><div className="modal-footer"><button className="secondary-button" onClick={()=>setPending(null)}>돌아가서 수정</button><button className="primary-button" onClick={()=>{if(submitting.current)return;submitting.current=true;const value=pending;setPending(null);onSave(value);}}>제출 완료</button></div></Overlay>:null;
  return {save,dialog};
}

type DeskTask={key:string;title:string;company:string;site?:string;period:string;due:string;status:string;reason?:string;editable:boolean;record?:ActivityRecord;metric?:MetricEntryContext;collection?:CollectionPeriod;scope?:Scope};
function WorkDesk({records,periods,requests,submissions,indicators,organizations,canManage,canWrite,onRecord,onMetric,factors,owner,department}:{records:ActivityRecord[];periods:CollectionPeriod[];requests:MetricRequest[];submissions:MetricSubmission[];indicators:Indicator[];organizations:Record<string,string[]>;canManage:boolean;canWrite:boolean;onRecord:(record:ActivityRecord)=>void;onMetric:(context:MetricEntryContext)=>void;factors:EmissionFactor[];owner:string;department:string}) {
  const [filter,setFilter]=useState(canWrite?"할 일":"제출 현황");const [company,setCompany]=useState("전체");const [search,setSearch]=useState("");
  const tasks:DeskTask[]=[];
  for(const collection of periods.filter(item=>item.status!=="예정"))for(const task of tasksAtSites(buildGHGCollectionTasks(collection),organizations,collection.sitesByCompany).filter(task=>Object.hasOwn(organizations,task.company))){
    const matches=records.filter(row=>row.active!==false&&row.collectionId===collection.id&&row.company===task.company&&(!task.site||row.site===task.site)&&row.period===task.period&&row.scope===task.targetId);
    if(matches.length)for(const record of matches)tasks.push({key:`ghg-${record.id}`,title:record.source,company:task.company,site:task.site,period:task.period,due:collection.dueDate,status:record.status,reason:record.rejectionReason,editable:collection.status==="수집중"&&!record.locked&&["작성중","반려"].includes(record.status),record,collection,scope:task.targetId});
    else tasks.push({key:`ghg-${collection.id}-${task.key}`,title:`${collection.name} · ${task.targetId}`,company:task.company,site:task.site,period:task.period,due:collection.dueDate,status:"미제출",editable:collection.status==="수집중",collection,scope:task.targetId});
  }
  for(const request of requests.filter(item=>item.status!=="예정"))for(const task of tasksAtSites(buildMetricCollectionTasks(request,indicators),organizations,request.sitesByCompany).filter(task=>Object.hasOwn(organizations,task.company))){
    const indicator=indicators.find(row=>row.id===task.targetId);if(!indicator)continue;
    const submission=submissions.find(row=>row.requestId===request.id&&row.company===task.company&&(!task.site||row.site===task.site)&&row.period===task.period&&row.indicatorId===task.targetId);
    tasks.push({key:`esg-${request.id}-${task.key}`,title:indicator.name,company:task.company,site:task.site,period:task.period,due:request.dueDate,status:submission?.status??"미제출",reason:submission?.rejectionReason,editable:request.status==="수집중"&&(!submission||["작성중","반려"].includes(submission.status)),metric:{request,indicator,company:task.company,site:task.site,period:task.period,submission}});
  }
  const todo=(task:DeskTask)=>["미제출","작성중","반려"].includes(task.status);
  const matches=(task:DeskTask)=>filter==="미제출·작성 중"&&["미제출","작성중"].includes(task.status)||filter==="전체"||filter==="할 일"&&todo(task)||filter==="제출 현황"&&["검토대기","확정"].includes(task.status)||filter==="마감 임박"&&todo(task)&&!!task.due&&daysUntil(task.due)<=3||task.status===filter;
  const filtered=tasks.filter(task=>(company==="전체"||company===task.company)&&matches(task)&&`${task.title} ${task.company} ${task.period}`.includes(search)).sort((a,b)=>(a.status==="반려"?-1:0)-(b.status==="반려"?-1:0)||a.due.localeCompare(b.due));
  const open=(task:DeskTask)=>{
    if(!canWrite||!task.editable)return;
    if(task.metric){onMetric(task.metric);return;}if(task.record){onRecord(task.record);return;}
    const factor=factors.find(item=>item.active&&item.scope===task.scope);if(!factor||!task.collection)return;
    onRecord({id:0,collectionId:task.collection.id,company:task.company,site:task.site??organizations[task.company]?.[0]??"",period:task.period,scope:factor.scope,category:factor.category,source:factor.source,usage:0,dataStatus:"missing",factorId:factor.id,unit:factor.activityUnit,factor:factor.value,emissions:0,owner,department,status:"작성중",evidence:"",active:true,updatedAt:nowLabel()});
  };
  return <section className="work-desk"><PageHeader eyebrow="WORKSPACE" title={canManage?"법인별 요청·제출 현황":canWrite?"내 할 일":"제출 현황"} description={canManage?"법인별 숫자를 누르면 해당 자료 목록으로 이동합니다.":"요청받은 자료를 선택하면 대상과 기간이 자동으로 채워집니다."}/><div className="desk-filters">{["할 일","반려","마감 임박","제출 현황","전체"].map(value=><button key={value} className={filter===value?"primary-button":"secondary-button"} onClick={()=>setFilter(value)}>{value==="반려"?"수정 요청":value} {tasks.filter(task=>value==="할 일"?todo(task):value==="반려"?task.status==="반려":value==="마감 임박"?todo(task)&&!!task.due&&daysUntil(task.due)<=3:value==="제출 현황"?["검토대기","확정"].includes(task.status):true).length}</button>)}</div>
  {canManage&&<div className="card table-scroll"><table className="data-table"><thead><tr><th>법인</th><th>미제출·작성 중</th><th>검토 중</th><th>수정 요청</th><th>승인 완료</th></tr></thead><tbody>{Object.keys(organizations).map(name=><tr key={name}><td>{name}</td>{["미제출","검토대기","반려","확정"].map(status=><td key={status}><button className="outline-small" onClick={()=>{setCompany(name);setFilter(status==="미제출"?"미제출·작성 중":status);document.getElementById("desk-list")?.scrollIntoView({behavior:"smooth",block:"start"});}}>{tasks.filter(task=>task.company===name&&(status==="미제출"?["미제출","작성중"].includes(task.status):task.status===status)).length}건</button></td>)}</tr>)}</tbody></table></div>}
  <div id="desk-list" className="card"><div className="data-toolbar"><input aria-label="요청 자료 검색" placeholder="자료명·자료 기준월 검색" value={search} onChange={event=>setSearch(event.target.value)}/>{canManage&&<select aria-label="법인 선택" value={company} onChange={event=>setCompany(event.target.value)}><option value="전체">전체 법인</option>{Object.keys(organizations).map(name=><option key={name}>{name}</option>)}</select>}<button className="outline-small" onClick={()=>{setFilter("전체");setCompany("전체");setSearch("");}}>전체 보기</button></div><div className="desk-tasks">{filtered.map(task=><article className="desk-task" key={task.key}><div><StatusBadge status={task.status}/><h3>{task.title}</h3><p>{task.company} · {task.site||task.record?.site||""} · {task.period}</p>{task.record&&<p>{formatNumber(task.record.usage,2)} {task.record.unit} · {formatNumber(task.record.emissions,2)} tCO₂e</p>}{task.metric?.submission&&<p>{task.metric.submission.detailRows?.length?`상세값 ${task.metric.submission.detailRows.length}개`: `${formatNumber(task.metric.submission.value,2)} ${task.metric.submission.unit}`}</p>}<small>{task.due?`${task.due} 마감${todo(task)&&daysUntil(task.due)<0?" · 기한 지남":""}`:"마감일 미지정"}</small>{task.reason&&<p className="desk-reason">수정 요청: {task.reason}</p>}</div>{canWrite&&task.editable?<button className="primary-button" disabled={!task.metric&&!task.record&&!factors.some(item=>item.active&&item.scope===task.scope)} onClick={()=>open(task)}>{task.status==="반려"?"수정 후 재제출":task.status==="작성중"?"이어서 작성":"입력하기"}</button>:<span>{task.status==="검토대기"?"검토 결과를 기다려 주세요":task.status==="확정"?"승인 완료":!canWrite?"조회 전용":"수집기간 종료"}</span>}</article>)}{!filtered.length&&<div className="empty-state"><Icon name="check"/><strong>조건에 해당하는 자료가 없습니다.</strong><p>전체 보기를 누르면 제출한 자료도 확인할 수 있습니다.</p></div>}</div></div></section>;
}

function SimpleSelect({children,...props}:ComponentProps<"select">){
  const options=Children.toArray(children).filter(child=>isValidElement(child)&&child.type==="option");
  if(options.length===1&&isValidElement<{value?:string;children?:ReactNode}>(options[0])){
    return <input readOnly className="readonly-input" aria-label={props["aria-label"]} value={Children.toArray(options[0].props.children).join("")||String(props.value??"")}/>;
  }
