...
function TargetForm({target,records,organizationNames,linkedPlans,onClose,onSave,onDelete}:{target:ReductionTarget|null;records:ActivityRecord[];organizationNames:string[];linkedPlans:number;onClose:()=>void;onSave:(target:ReductionTarget)=>void;onDelete?:()=>void}){
  ...
  return <Overlay title={target?"감축목표 수정":"새 감축목표 설정"} eyebrow="REDUCTION TARGET" description="기준값은 확정 인벤토리에서 불러오며 목표배출량과 필요 감축량은 자동 계산됩니다." onClose={onClose}><form onSubmit={submit}><div className="form-section"><h3><span>1</span>목표 범위</h3><div className="form-grid"><label className="full-span">목표명<input value={form.name} onChange={event=>recalculate({name:event.target.value})} placeholder="예: 그룹 Scope 1·2 2030 감축목표" required/></label><label>대상 조직<select value={form.company} onChange={event=>recalculate({company:event.target.value},true)}><option>그룹 전체</option>{organizationNames.map(company=><option key={company}>{company}</option>)}</select></label><label>목표 담당부서<input value={form.owner} onChange={event=>recalculate({owner:event.target.value})} required/></label></div><div className="check-group"><strong>대상 Scope</strong><div>{(["Scope 1","Scope 2","Scope 3"] as Scope[]).map(scope=><label key={scope}><input type="checkbox" checked={form.scopes.includes(scope)} onChange={()=>toggleScope(scope)}/>{scope}</label>)}</div></div></div>
