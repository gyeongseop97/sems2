  ...
  return <><PageHeader eyebrow="COLLECTION PERIOD" title="수집 기간 관리" description="귀속기간, 대상 법인·Scope, 제출·검토 마감과 잠금 상태를 운영합니다.">{canManage&&<button className="primary-button" onClick={() => setEditing("new")}><Icon name="plus" size={17}/>수집기간 개설</button>}</PageHeader>
    ...
    <section className="period-grid">{periods.map(period => {
      ...
      return <article className="card period-card" key={period.id}><div className="period-card-top"><div><span className={`status-badge ${periodTone(period.status)}`}><span className="status-dot"/>{period.status}</span><h2>{period.name}</h2><p>{period.description}</p></div></div>
        <div className="period-dates"><div><span>귀속기간</span><strong>{period.dataFrom === period.dataTo ? period.dataFrom : `${period.dataFrom} ~ ${period.dataTo}`}</strong></div><div><span>수집기간</span><strong>{period.openDate} ~ {period.dueDate}{koreanHolidayName(period.dueDate)&&<em style={{display:"block",marginTop:4,fontStyle:"normal",fontSize:12,fontWeight:700,color:"#c94b4b"}}>{koreanHolidayName(period.dueDate)}</em>}</strong></div><div><span>검토마감</span><strong>{period.reviewDate}{koreanHolidayName(period.reviewDate)&&<em style={{display:"block",marginTop:4,fontStyle:"normal",fontSize:12,fontWeight:700,color:"#c94b4b"}}>{koreanHolidayName(period.reviewDate)}</em>}</strong></div></div>
