...
import { Children, ComponentProps, isValidElement, ChangeEvent, CSSProperties, DragEvent as ReactDragEvent, FormEvent, PointerEvent as ReactPointerEvent, ReactNode, useDeferredValue, useEffect, useMemo, useRef, useState } from "react";
...
function Review({records,periods,criteria,actor,onChange,showToast}:{records:ActivityRecord[];periods:CollectionPeriod[];criteria:CollectionCriteria;actor:string;onChange:(records:ActivityRecord[],auditInfo?:{action:string;target:string;detail:string})=>void;showToast:(m:string)=>void}){
  ...
  const handleAction=(id:number,action:"approve"|"reject"|"note",note:string)=>{
    const selected=activeRecords.find(record=>record.id===id);if(!selected)return;
    if(periods.find(period=>period.id===selected.collectionId)?.status==="잠금"){showToast("잠금된 수집기간의 자료는 처리할 수 없습니다.");return;}
    if(action!=="note"&&selected.status!=="검토대기"){showToast("검토대기 자료만 처리할 수 있습니다.");return;}
    ...
    const at=new Date().toISOString();const label=action==="approve"?"자료 확정":action==="reject"?"보완 요청":"검토 의견";
    onChange(records.map(record=>record.id===id?{...record,status:action==="approve"?"확정":action==="reject"?"반려":record.status,locked:action==="approve"?criteria.lockConfirmed:action==="reject"?false:record.locked,rejectionReason:action==="reject"?note:action==="approve"?"":record.rejectionReason,reviewNote:note,reviewedAt:at,reviewedBy:actor,reviewHistory:[...(record.reviewHistory??[]),{at,actor,action:label,note}],updatedAt:nowLabel()}:record),{action:label,target:selected.company+" · "+selected.source,detail:note||selected.period+" 자료를 검토했습니다."});
    showToast("검토 결과를 서버에 저장 중입니다. 상단 저장 상태를 확인해 주세요.");
  };
  return <><PageHeader eyebrow="REVIEW & APPROVAL" title="검토·승인 작업함" description="사업장별 제출값과 계산 근거를 확인하고, 행별 검토 의견과 보완 요청을 남깁니다."/><OperationsWorkbench rows={activeRecords.map(record=>({ ...record,title:record.category+" · "+record.source,value:formatNumber(record.emissions,2)+" tCO₂e",dataStatus:DATA_STATUS_LABELS[dataValueStatus({value:record.usage,dataStatus:record.dataStatus})],locked:periods.find(period=>period.id===record.collectionId)?.status==="잠금",resubmitted:record.reviewHistory?.some(item=>item.action==="보완 요청"),facts:[{label:"활동량",value:formatNumber(record.usage,4)+" "+record.unit},{label:"저장된 배출계수",value:String(record.factor)},{label:"계수 식별자",value:record.factorId||"기존 자료 · 기록 없음"},{label:"계수 적용 기준",value:record.factorSnapshot?[record.factorSnapshot.year+"년",record.factorSnapshot.authority,record.factorSnapshot.factorUnit,"버전 "+record.factorSnapshot.revision,"유효기간 "+(record.factorSnapshot.validFrom||"시작 제한 없음")+" ~ "+(record.factorSnapshot.validTo||"종료 제한 없음")].join(" · "):"기존 저장값 유지 · 신규 입력부터 계수 버전 보존"}],checks:activityChecks(record,records,criteria),history:record.reviewHistory}))} onAction={handleAction}/></>;
}
