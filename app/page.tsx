...
function RecordModal({record,records,periods,factors,criteria,organizations,defaultOwner,defaultDepartment,onClose,onSave}:{record:ActivityRecord|null;records:ActivityRecord[];periods:CollectionPeriod[];factors:EmissionFactor[];criteria:CollectionCriteria;organizations:Record<string,string[]>;defaultOwner:string;defaultDepartment:string;onClose:()=>void;onSave:(r:ActivityRecord)=>void}){
  ...
  const submit=(e:FormEvent)=>{e.preventDefault();if(!selectedPeriod||selectedPeriod.status!=="수집중"){setError("현재 수집중인 기간을 선택해 주세요.");return;}if(!selectedTasks.some(task=>task.company===form.company&&task.targetId===form.scope&&task.period===form.period)){setError("이 요청에 생성된 세부 수집 항목을 선택해 주세요.");return;}if(!Number.isFinite(form.usage)||form.usage<=0){setError("사용량을 입력해 주세요.");return;}if(duplicate){setError("같은 사업장·자료 기준월·활동자료가 이미 등록되어 있습니다.");return;}review.save({...form,status:submissionIntent(e),rejectionReason:submissionIntent(e)==="검토대기"?"":form.rejectionReason,emissions:Math.round(form.usage*form.factor/1000*100)/100,updatedAt:"방금 전"},submissionIntent(e));};
