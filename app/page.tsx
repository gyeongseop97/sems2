...
function CollectionCoverage({
  ...
  return <><PageHeader eyebrow="COLLECTION COVERAGE" title="요청·제출 현황 현황" description="연도·월·법인·수집 항목별로 요청 누락과 입력·검토·확정 상태를 확인합니다."/>
...
function Periods({ periods, records, organizationNames, onChange, addAudit, showToast }: { periods: CollectionPeriod[]; records: ActivityRecord[]; organizationNames: string[]; onChange: (items: CollectionPeriod[]) => void; addAudit: (action: string, target: string, detail: string, actor?: string) => void; showToast: (m: string) => void }) {
  ...
  const updateStatus = (period: CollectionPeriod, status: PeriodStatus) => {
    ...
    const requestedTasks = buildGHGCollectionTasks(period);
    const confirmedTaskKeys = new Set(periodRecords.filter(record => record.status === "확정").map(record => collectionTaskKey(record.company, record.scope, record.period)));
    const missingTasks = requestedTasks.filter(task => !confirmedTaskKeys.has(task.key));
    if (status === "마감" && (!periodRecords.length || unresolved.length || missingTasks.length)) {
      showToast(!periodRecords.length ? "등록된 자료가 없어 검토를 마감할 수 없습니다." : missingTasks.length ? `요청된 수집 항목 중 확정되지 않은 항목이 ${missingTasks.length}건 있습니다.` : `미확정 자료 ${unresolved.length}건을 모두 확정하거나 제외해 주세요.`);
      return;
    }
...
function Review({ records, periods, criteria, onChange, showToast }: { records: ActivityRecord[]; periods: CollectionPeriod[]; criteria: CollectionCriteria; onChange: (records: ActivityRecord[], auditInfo?: { action: string; target: string; detail: string }) => void; showToast: (m: string) => void }) {
  ...
  return <><PageHeader eyebrow="REVIEW & APPROVAL" title="검토·승인" description="제출된 활동자료와 증빙, 전월·전년 값을 대조해 확정하거나 보완을 요청합니다."><button className="secondary-button" onClick={()=>{downloadCsv("SEMS_review_queue.csv",["법인","사업장","자료 기준월","Scope","활동자료","사용량","배출량","증빙","담당자"],visible.map(record=>[record.company,record.site,record.period,record.scope,record.source,record.usage,record.emissions,record.evidence,record.owner]));showToast("검토 대기 목록을 내려받았습니다.");}}><Icon name="download" size={17}/>대기목록 내보내기</button></PageHeader>
    <section className="review-workspace"><aside className="card review-queue"><CardHeader title={`검토 대기 ${queue.length}건`} subtitle="제출 순서대로 확인하세요."/><div className="queue-search search-box"><Icon name="search" size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="법인, 배출원, 담당자 검색"/></div><div className="queue-list">{visible.map(record=><button key={record.id} className={selected?.id===record.id?"active":""} onClick={()=>{setSelectedId(record.id);setReason("")}}><div className={`source-icon ${record.scope==="Scope 1"?"green":"blue"}`}><Icon name={record.scope==="Scope 1"?"droplet":"bolt"} size={17}/></div><div><strong>{record.source}</strong><span>{record.company} · {record.period}</span></div><em>{formatNumber(record.emissions,2)} t</em></button>)}{!visible.length&&<div className="empty-state compact"><Icon name="check"/><strong>검토 대기 자료가 없습니다.</strong></div>}</div></aside>
      <article className="card review-detail">{selected ? <><div className="review-detail-head"><div><span className={`scope-tag s${selected.scope.slice(-1)}`}>{selected.scope}</span><h2>{selected.category} · {selected.source}</h2><p>{selected.company} / {selected.site} · 자료 기준월 {selected.period}</p></div><StatusBadge status={selected.status}/></div>
...
function DataQuality({records,periods,criteria,onNavigate}:{records:ActivityRecord[];periods:CollectionPeriod[];criteria:CollectionCriteria;onNavigate:(view:View)=>void}){
  ...
  for(const record of activeRecords){
    ...
    const expectedEmissions=Math.round(record.usage*record.factor/1000*100)/100;
    if(!Number.isFinite(record.emissions)||Math.abs(expectedEmissions-record.emissions)>0.01)issues.push({key:`${base}-calculation`,severity:"오류",rule:"배출량 산식 불일치",record,detail:`사용량 × 배출계수로 계산한 ${formatNumber(expectedEmissions,2)} tCO₂e와 저장값 ${formatNumber(record.emissions,2)} tCO₂e가 다릅니다.`});
    ...
    if((duplicateCounts.get(duplicateKey)??0)>1)issues.push({key:`${base}-duplicate`,severity:"오류",rule:"중복 입력",record,detail:"같은 수집기간·사업장·자료 기준월·배출원의 자료가 중복되었습니다."});
  ...
  return <><PageHeader eyebrow="DATA QUALITY" title="입력 오류·확인 사항" description="누락·중복·단위·급변·반려 자료를 자동으로 찾아 검토 대상을 먼저 보여줍니다."><button className="primary-button" onClick={()=>onNavigate("collection")}><Icon name="database" size={17}/>원천자료 확인</button></PageHeader>
...
function MetricSubmissionForm({context,organizations,defaultOwner,defaultDepartment,canManage,onClose,onSave}:{context:{request:MetricRequest;indicator:Indicator;company:string;period:string;submission?:MetricSubmission};organizations:Record<string,string[]>;defaultOwner:string;defaultDepartment:string;canManage:boolean;onClose:()=>void;onSave:(submission:MetricSubmission)=>void}){
  ...
  const submit=(event:FormEvent)=>{
    ...
    review.save({...form,status:submissionIntent(event),rejectionReason:submissionIntent(event)==="검토대기"?"":form.rejectionReason,detailRows:template==="GENERAL"?undefined:rows,value:template==="GENERAL"?form.value:template==="FIXED"?0:calculateMetricDetailTotal(template,rows)}, submissionIntent(event));
  };
...
function BatchRecordEntry({records,periods,factors,organizations,defaultOwner,defaultDepartment,onClose,onSave,showToast}:{records:ActivityRecord[];periods:CollectionPeriod[];factors:EmissionFactor[];organizations:Record<string,string[]>;defaultOwner:string;defaultDepartment:string;onClose:()=>void;onSave:(rows:ActivityRecord[])=>void;showToast:(message:string)=>void}){
  ...
  const submit=(event:FormEvent)=>{event.preventDefault();if(!selectedPeriod||!selectedTasks.some(task=>task.company===context.company&&task.targetId===context.scope&&task.period===context.period)){setError("현재 수집 요청에 포함된 회사·Scope·자료 기준월을 선택해 주세요.");return;}if(!context.site){setError("사업장을 선택해 주세요.");return;}if(!rows.length||rows.some(row=>!factorFor(row)||!Number.isFinite(row.usage)||row.usage<=0)){setError("모든 행의 활동자료와 0보다 큰 사용량을 입력해 주세요.");return;}if(duplicateIdentities.size){setError("입력표 안에 같은 배출원이 두 번 들어 있습니다. 중복 행을 정리해 주세요.");return;}if(rows.some(row=>existingIdentities.has(rowIdentity(row)))){setError("이미 저장된 같은 사업장·자료 기준월·배출원이 있습니다. 기존 행을 수정하거나 다른 배출원을 선택해 주세요.");return;}if(!context.owner.trim()||!context.department.trim()){setError("공통 담당자와 담당 부서를 입력해 주세요.");return;}review.save(rows.map(row=>{const factor=factorFor(row)!;return {id:0,collectionId:context.collectionId,company:context.company,site:context.site,period:context.period,scope:context.scope,category:factor.category,source:factor.source,usage:row.usage,unit:factor.activityUnit,factor:factor.value,emissions:Math.round(row.usage*factor.value/1000*100)/100,owner:context.owner.trim(),department:context.department.trim(),status:submissionIntent(event),evidence:row.evidence,description:row.description,active:true,updatedAt:"방금 전"};}),submissionIntent(event));};
...
function RecordModal({record,records,periods,factors,criteria,organizations,defaultOwner,defaultDepartment,onClose,onSave}:{record:ActivityRecord|null;records:ActivityRecord[];periods:CollectionPeriod[];factors:EmissionFactor[];criteria:CollectionCriteria;organizations:Record<string,string[]>;defaultOwner:string;defaultDepartment:string;onClose:()=>void;onSave:(r:ActivityRecord)=>void}){
  ...
  const submit=(e:FormEvent)=>{e.preventDefault();if(!canCollect(selectedPeriod)){setError("현재 수집중인 기간을 선택해 주세요.");return;}if(!selectedTasks.some(task=>task.company===form.company&&task.targetId===form.scope&&task.period===form.period)){setError("이 요청에 생성된 세부 수집 항목을 선택해 주세요.");return;}if(!Number.isFinite(form.usage)||form.usage<=0){setError("사용량을 입력해 주세요.");return;}if(duplicate){setError("같은 사업장·자료 기준월·활동자료가 이미 등록되어 있습니다.");return;}review.save({...form,status:submissionIntent(e),rejectionReason:submissionIntent(e)==="검토대기"?"":form.rejectionReason,emissions:Math.round(form.usage*form.factor/1000*100)/100,updatedAt:"방금 전"},submissionIntent(e));};
  ...
  return <Overlay title={record?"활동자료 수정":"신규 활동자료 입력"} eyebrow="ACTIVITY DATA" description="수집기간과 Scope에 맞는 활동자료·배출계수가 자동 연결됩니다." onClose={onClose}><GuidedForm onSubmit={submit}>
    ...
    <div className="modal-footer"><button type="button" className="secondary-button" onClick={onClose}>취소</button><SubmissionButtons rejected={record?.status==="반려"} disabled={!available.length||duplicate}/></div></GuidedForm>{review.dialog}</Overlay>;
}

type MetricEntryContext={request:MetricRequest;indicator:Indicator;company:string;period:string;submission?:MetricSubmission};
...
type DeskTask={key:string;title:string;company:string;period:string;due:string;status:string;reason?:string;editable:boolean;record?:ActivityRecord;metric?:MetricEntryContext;collection?:CollectionPeriod;scope?:Scope};
function WorkDesk({records,periods,requests,submissions,indicators,organizations,canManage,canWrite,onRecord,onMetric,factors,owner,department}:{records:ActivityRecord[];periods:CollectionPeriod[];requests:MetricRequest[];submissions:MetricSubmission[];indicators:Indicator[];organizations:Record<string,string[]>;canManage:boolean;canWrite:boolean;onRecord:(record:ActivityRecord)=>void;onMetric:(context:MetricEntryContext)=>void;factors:EmissionFactor[];owner:string;department:string}) {
  ...
  for(const collection of periods.filter(item=>item.status!=="예정"))for(const task of buildGHGCollectionTasks(collection).filter(task=>Object.hasOwn(organizations,task.company))){
    const matches=records.filter(row=>row.active!==false&&row.collectionId===collection.id&&row.company===task.company&&row.period===task.period&&row.scope===task.targetId);
    if(matches.length)for(const record of matches)tasks.push({key:`ghg-${record.id}`,title:record.source,company:task.company,period:task.period,due:collection.dueDate,status:record.status,reason:record.rejectionReason,editable:collection.status==="수집중"&&!record.locked&&["작성중","반려"].includes(record.status),record,collection,scope:task.targetId});
    else tasks.push({key:`ghg-${collection.id}-${task.key}`,title:`${collection.name} · ${task.targetId}`,company:task.company,period:task.period,due:collection.dueDate,status:"미제출",editable:collection.status==="수집중",collection,scope:task.targetId});
  }
  for(const request of requests.filter(item=>item.status!=="예정"))for(const task of buildMetricCollectionTasks(request,indicators).filter(task=>Object.hasOwn(organizations,task.company))){
    const indicator=indicators.find(row=>row.id===task.targetId);if(!indicator)continue;
    const submission=submissions.find(row=>row.requestId===request.id&&row.company===task.company&&row.period===task.period&&row.indicatorId===task.targetId);
    tasks.push({key:`esg-${request.id}-${task.key}`,title:indicator.name,company:task.company,period:task.period,due:request.dueDate,status:submission?.status??"미제출",reason:submission?.rejectionReason,editable:request.status==="수집중"&&(!submission||["작성중","반려"].includes(submission.status)),metric:{request,indicator,company:task.company,period:task.period,submission}});
  }
  ...
  const open=(task:DeskTask)=>{
    ...
    onRecord({id:0,collectionId:task.collection.id,company:task.company,site:organizations[task.company]?.[0]??"",period:task.period,scope:factor.scope,category:factor.category,source:factor.source,usage:0,unit:factor.activityUnit,factor:factor.value,emissions:0,owner,department,status:"작성중",evidence:"",active:true,updatedAt:nowLabel()});
  };
