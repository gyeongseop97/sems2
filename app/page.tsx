...
export default function Home() {
  const { profile, syncStatus, canWrite, canReview, canManage, isAdmin, signOut } = useSemsAuth();
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      <header className="topbar"><button className="icon-button mobile-menu-button" onClick={() => setMobileMenu(true)} aria-label="메뉴 열기"><Icon name="menu" /></button><div className="breadcrumb"><span>SEMS</span><Icon name="chevron" size={14} /><strong>{navItems.find(n => n.id === activeView)?.label ?? "시스템 설정"}</strong></div><div className="topbar-actions"><div className={`sync-label operating ${syncStatus}`}><span /> {syncLabel}</div><button className="icon-button notification-button" onClick={() => { setNotificationsOpen(!notificationsOpen); setProfileOpen(false); }} aria-label="알림"><Icon name="bell" />{!notificationsRead && <span className="notification-dot" />}</button><button className="profile profile-button" onClick={() => { setProfileOpen(!profileOpen); setNotificationsOpen(false); }}><div className="avatar">{(profile.display_name || profile.email || "S").slice(0,1)}</div><div><strong>{profile.display_name || profile.email}</strong><span>{profile.department || profile.organization?.name || "소속 미지정"} · {roleLabel}</span></div><Icon name="chevron" size={15} /></button></div>
        ...
        {profileOpen && <ProfilePanel profileName={profile.display_name || profile.email || "사용자"} detail={`${profile.department || profile.organization?.name || "소속 미지정"} · ${roleLabel}`} canManage={canManage} isAdmin={isAdmin} onSettings={() => navigate("settings")} onBackup={exportBackup} onSignOut={signOut} />}
      </header>
...
function ProfilePanel({profileName,detail,canManage,isAdmin,onSettings,onBackup,onSignOut}:{profileName:string;detail:string;canManage:boolean;isAdmin:boolean;onSettings:()=>void;onBackup:()=>void;onSignOut:()=>Promise<void>}){return <div className="profile-panel"><div><strong>{profileName}</strong><span>{detail}</span></div>{canManage&&<button onClick={onSettings}><Icon name="settings" size={16}/>시스템 설정</button>}<button onClick={onBackup}><Icon name="download" size={16}/>{isAdmin?"전체 데이터 백업":"내 법인 데이터 백업"}</button><button className="profile-logout" onClick={()=>void onSignOut()}><Icon name="logout" size={16}/>로그아웃</button><p>운영 데이터는 Supabase 서버에 자동 저장되며 계정 권한에 따라 조회·수정 범위가 제한됩니다.</p></div>}
...
function PermissionSettings({showToast}:{showToast:(m:string)=>void}){
  const roles=[{name:"관리자",desc:"모든 법인의 모든 데이터 조회·수정과 검토·승인, 기준정보·사용자 관리",write:true,approve:true},{name:"자료 입력자",desc:"소속 법인 데이터 조회와 요청된 활동자료·정량데이터·증빙 입력 및 제출",write:true,approve:false},{name:"조회자",desc:"소속 법인의 데이터만 조회",write:false,approve:false}];
