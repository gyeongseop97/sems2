...
export default function Home() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        const savedMetricRequests = localStorage.getItem("sems2-metric-requests"); if (savedMetricRequests) setMetricRequests((JSON.parse(savedMetricRequests) as MetricRequest[]).map(request=>({...request,indicatorIds:[...new Set(request.indicatorIds.map(id=>indicatorIdMap.get(id)??id))]})));
        const savedMetricSubmissions = localStorage.getItem("sems2-metric-submissions"); if (savedMetricSubmissions) setMetricSubmissions((JSON.parse(savedMetricSubmissions) as MetricSubmission[]).map(submission=>({...submission,indicatorId:indicatorIdMap.get(submission.indicatorId)??submission.indicatorId})));
  ...
  const restoreBackup = (payload: Record<string, unknown>) => {
    ...
    if (Array.isArray(payload.evidence)) setEvidence(payload.evidence as EvidenceItem[]); if (Array.isArray(payload.indicators)) setIndicators(normalizeMetricIndicators(mergeMasterRows(payload.indicators as Indicator[],DEFAULT_MASTER_INDICATORS))); if (Array.isArray(payload.audit)) setAudit(payload.audit as AuditEvent[]);
...
function normalizeMetricIndicators(items:Indicator[]):Indicator[]{
  const filtered=items.filter(item=>!REDUNDANT_METRIC_CODES.has(item.code));
...
function createMetricIndicatorIdMap(source:Indicator[],normalized:Indicator[]):Map<number,number>{
  ...
  return new Map(source.flatMap(item=>{
    ...
    return replacementId===undefined?[]:[[item.id,replacementId] as [number,number]];
  }));
}
...
function MetricSubmissionForm({context,organizations,defaultOwner,defaultDepartment,canManage,onClose,onSave}:{context:{request:MetricRequest;indicator:Indicator;company:string;submission?:MetricSubmission};organizations:Record<string,string[]>;defaultOwner:string;defaultDepartment:string;canManage:boolean;onClose:()=>void;onSave:(submission:MetricSubmission)=>void}){
  ...
  const [form,setForm]=useState<MetricSubmission>(existing??{id:0,requestId:context.request.id,indicatorId:context.indicator.id,company:context.company,site:organizations[context.company]?.[0]??"",period:context.request.periodTo,value:0,unit:context.indicator.unit,owner:defaultOwner,department:defaultDepartment,evidence:"",description:"",status:"작성중",detailRows:template==="GENERAL"?undefined:[createMetricDetailRow(template)],employeeCount:template==="TRAINING"?0:undefined,updatedAt:"방금 전"});
