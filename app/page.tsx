...
export default function Home() {
  ...
  const [factors, setFactors] = useState<EmissionFactor[]>(DEFAULT_EMISSION_FACTORS);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        const savedFactors = localStorage.getItem("sems2-factors"); if (savedFactors) { const parsed = JSON.parse(savedFactors) as EmissionFactor[]; setFactors(parsed.length ? parsed : DEFAULT_EMISSION_FACTORS); }
        const savedFormulas = localStorage.getItem("sems2-formulas"); if (savedFormulas) { const parsed = JSON.parse(savedFormulas) as CalculationFormula[]; setFormulas(parsed.length ? parsed : DEFAULT_CALCULATION_FORMULAS); }
...
function normalizeReportDefaults(items:SustainabilityReport[]):SustainabilityReport[]{
  return items.map(report=>({
    ...
    pages:report.pages.map(page=>{
      const normalizedBlocks=page.blocks.map(block=>{
        ...
        return {...block,fontSize:block.fontSize===undefined||block.fontSize===16?REPORT_BODY_SIZE:block.fontSize};
      });
...
function createReportPages(reportStyle:Pick<SustainabilityReport,"primaryColor"|"accentColor">):ReportPage[]{
  ...
  return [
    ...
    page("환경 Environmental","환경",[block("text","환경경영","환경경영 추진체계와 주요 정책, 목표 및 활동을 작성해 주세요.",undefined,0),block("data","온실가스 배출 실적","확정된 인벤토리 데이터를 자동으로 표시합니다.","온실가스 배출량",1)]),
    ...
    page("ESG Data Factbook","부록",[block("data","온실가스 배출량","확정된 연도별 Scope 데이터를 표시합니다.","온실가스 배출량",0),block("data","감축목표","등록된 중장기 감축목표를 표시합니다.","감축목표",1),block("data","ESG 지표 현황","지표 정의서의 수집·승인 상태를 표시합니다.","ESG 지표",2)]),
...
function ReportBuilder({reports,records,targets,indicators,standards,organizationNames,canManage,onChange,addAudit,showToast}:{reports:SustainabilityReport[];records:ActivityRecord[];targets:ReductionTarget[];indicators:Indicator[];standards:DisclosureStandard[];organizationNames:string[];canManage:boolean;onChange:(items:SustainabilityReport[])=>void;addAudit:(action:string,target:string,detail:string,actor?:string)=>void;showToast:(message:string)=>void}){
...
function ReportCanvasEditor({reports,report,page,canManage,records,targets,indicators,onReportSelect,onPageSelect,onPagePatch,onPageAdd,onPageReorder,onPageRemove,addAudit,showToast}:{reports:SustainabilityReport[];report:SustainabilityReport;page?:ReportPage;canManage:boolean;records:ActivityRecord[];targets:ReductionTarget[];indicators:Indicator[];onReportSelect:(id:string)=>void;onPageSelect:(id:string)=>void;onPagePatch:(patch:Partial<ReportPage>)=>void;onPageAdd:()=>void;onPageReorder:(pageId:string,targetId:string,placement:"before"|"after")=>void;onPageRemove:()=>void;addAudit:(action:string,target:string,detail:string,actor?:string)=>void;showToast:(message:string)=>void}){
  ...
  const addBlock=(type:ReportBlockType,headingStyle:ReportHeadingStyle="major")=>{
    ...
    const base:ReportBlock={id:`RB-${crypto.randomUUID()}`,type,title,body,dataSource:type==="data"||type==="chart"?"온실가스 배출량":undefined,x:7+(index%3)*3,y:12+(index%4)*7,w:type==="line"||type==="divider"?86:type==="title"?70:44,h:type==="line"||type==="divider"?5:type==="title"?10:type==="data"?30:24,fontSize:type==="title"?REPORT_HEADING_SIZES.major:REPORT_BODY_SIZE,textAlign:"left",color:"#263832",backgroundColor:type==="callout"?report.accentColor:"#ffffff",border:type==="callout",imageFit:"cover",chartType:"bar"};
...
function ReportDataChart({source,report,records,targets,indicators,chartType}:{source:ReportDataSource;report:SustainabilityReport;records:ActivityRecord[];targets:ReductionTarget[];indicators:Indicator[];chartType:"bar"|"line"}){
  const years=reportYears(report);let values:{label:string;value:number}[]=[];
  ...
  else values=(["환경","사회","지배구조"] as const).map(category=>{const rows=indicators.filter(indicator=>indicator.category===category&&indicator.active);return {label:category,value:rows.length?rows.reduce((sum,row)=>sum+row.progress,0)/rows.length:0};});
  const max=Math.max(1,...values.map(item=>item.value));
  return <div className={`report-mini-chart ${chartType}`}>{values.map(item=><div key={item.label}><span style={{height:`${Math.max(3,item.value/max*100)}%`,background:report.primaryColor}}><em>{formatNumber(item.value,1)}</em></span><small>{item.label}</small></div>)}</div>;
}
...
function ReferenceFactorManager({items,canManage,onChange,addAudit,showToast}:{items:EmissionFactor[];canManage:boolean;onChange:(x:EmissionFactor[])=>void;addAudit:(a:string,t:string,d:string)=>void;showToast:(m:string)=>void}){
  const [editing,setEditing]=useState<EmissionFactor|null|"new">(null);const restore=()=>{const next=mergeDefaultEmissionFactors(items) as EmissionFactor[];onChange(next);showToast(next.length===items.length?"기본 배출계수가 모두 등록되어 있습니다.":`기본 배출계수 ${next.length-items.length}개를 복원했습니다.`);};
  ...
  return <><ReferencePanel title="Scope 1·2·3 배출계수" subtitle="공식·공급자·참고계수의 값, 단위, 연도, 산정방법, 출처와 적용 주의사항을 관리합니다." action={<div className="row-actions"><button className="outline-small" disabled={!canManage} onClick={restore}><Icon name="refresh" size={14}/>기본계수 복원</button><button className="primary-button compact" disabled={!canManage} onClick={()=>setEditing("new")}><Icon name="plus" size={14}/>계수 추가</button></div>}><div className="table-scroll"><table className="data-table reference-table"><thead><tr><th>Scope</th><th>활동자료·배출원</th><th>배출계수</th><th>유형·연도</th><th>출처·산정방법</th><th>상태</th><th></th></tr></thead><tbody>{items.map(item=><tr key={item.id}><td><span className={`scope-tag s${item.scope.slice(-1)}`}>{item.scope}</span></td><td><strong>{item.source}</strong><small>{item.category}</small></td><td className="mono"><strong>{formatNumber(item.value,item.value<10?5:1)}</strong><small>{item.factorUnit}</small></td><td>{item.factorType??"운영계수"}<small>{item.year}</small></td><td><strong>{item.reference||item.authority}</strong><small>{item.method||"활동량 × 배출계수"}</small></td><td><StatusBadge status={item.active?"사용":"중지"}/></td><td><button className="outline-small" disabled={!canManage} onClick={()=>setEditing(item)}>수정</button></td></tr>)}</tbody></table></div></ReferencePanel>{editing&&<FactorForm factor={editing==="new"?null:editing} onClose={()=>setEditing(null)} onSave={save} onDelete={editing==="new"?undefined:()=>{const item=editing as EmissionFactor;if(!window.confirm("이 배출계수를 삭제하시겠습니까?"))return;onChange(items.filter(row=>row.id!==item.id));setEditing(null);showToast("배출계수를 삭제했습니다.");}}/>}</>;
}
...
function RegulationManager({items,standards,indicators,canManage,onChange,addAudit,showToast}:{items:ComplianceRegulation[];standards:DisclosureStandard[];indicators:Indicator[];canManage:boolean;onChange:(x:ComplianceRegulation[])=>void;addAudit:(a:string,t:string,d:string)=>void;showToast:(m:string)=>void}){
  ...
  return <ReferencePanel title="규제·준수 관리" subtitle="규제별 적용 여부, 담당자, 검토주기, 다음 검토일, 공시기준·ESG 지표·근거 증빙을 연결합니다."><div className="regulation-layout"><div className="reference-list regulation-list"><div className="reference-list-head"><strong>규제 목록</strong><button className="outline-small" disabled={!canManage} onClick={()=>{setDraft(empty);setTagText("")}}><Icon name="plus" size={14}/>새 규제</button></div>{items.map(item=><button key={item.id} className={draft.id===item.id?"active":""} onClick={()=>{setDraft(item);setTagText("")}}><span className="regulation-marker"><Icon name="alert" size={16}/></span><div><strong>{item.title}</strong><small>{item.jurisdiction} · {item.owner||"담당자 미지정"}</small></div><StatusBadge status={dueTone(item)}/></button>)}{!items.length&&<ReferenceEmpty icon="alert" title="등록된 규제가 없습니다." description="적용 대상 규제와 검토주기를 등록해 주세요."/>}</div><form className="reference-form regulation-form" onSubmit={save}><ReferenceFormTitle title={draft.id?"규제정보 수정":"새 규제"} description="법령 원문 대신 내부 적용 판단과 준수 활동·증빙 연결정보를 관리합니다."/><div className="form-grid"><label className="full-span">규제·법령명<input value={draft.title} onChange={e=>patch({title:e.target.value})} required/></label><label>분류<input value={draft.category} onChange={e=>patch({category:e.target.value})}/></label><label>관할<input value={draft.jurisdiction} onChange={e=>patch({jurisdiction:e.target.value})}/></label><label>적용 상태<select value={draft.status} onChange={e=>patch({status:e.target.value as ComplianceRegulation["status"]})}><option>검토 필요</option><option>적용</option><option>미적용</option><option>개정 검토</option></select></label><label>담당자<input value={draft.owner} onChange={e=>patch({owner:e.target.value})}/></label><label>검토주기(개월)<input type="number" min="1" value={draft.reviewCycleMonths} onChange={e=>patch({reviewCycleMonths:Number(e.target.value)})}/></label><label>최근 검토일<input type="date" value={draft.lastReviewDate} onChange={e=>patch({lastReviewDate:e.target.value})}/></label><label>다음 검토일<input type="date" value={draft.nextReviewDate} onChange={e=>patch({nextReviewDate:e.target.value})}/></label><label className="full-span textarea-label">주요 요구사항<textarea value={draft.contents} onChange={e=>patch({contents:e.target.value})}/></label><label className="full-span textarea-label">적용 판단·조치<textarea value={draft.applicability} onChange={e=>patch({applicability:e.target.value})}/></label><label className="full-span">증빙·근거 위치<input value={draft.evidence} onChange={e=>patch({evidence:e.target.value})} placeholder="증빙자료명, 내부 규정, 파일 경로"/></label><label className="full-span">태그<input value={tagText} onChange={e=>setTagText(e.target.value)} placeholder={`${draft.tags.join(", ")}${draft.tags.length?" · 새 태그를 쉼표로 추가":""}`}/></label></div><div className="reference-links"><div><strong>연결 보고기준</strong>{standards.map(item=><label key={item.id}><input type="checkbox" checked={draft.linkedStandardIds.includes(item.id)} onChange={()=>patch({linkedStandardIds:toggle(draft.linkedStandardIds,item.id)})}/><span>{item.code} · {item.title}</span></label>)}{!standards.length&&<p>등록된 보고기준이 없습니다.</p>}</div><div><strong>연결 ESG 지표</strong>{indicators.slice(0,12).map(item=><label key={item.id}><input type="checkbox" checked={draft.linkedIndicatorCodes.includes(item.code)} onChange={()=>patch({linkedIndicatorCodes:toggle(draft.linkedIndicatorCodes,item.code)})}/><span>{item.code} · {item.name}</span></label>)}{!indicators.length&&<p>등록된 ESG 지표가 없습니다.</p>}</div></div><Toggle label="준수 관리에 사용" checked={draft.active} onChange={active=>patch({active})}/><ReferenceFormActions canManage={canManage} editing={Boolean(draft.id)} onDelete={()=>{if(!draft.id||!window.confirm("이 규제 기준을 삭제하시겠습니까?"))return;onChange(items.filter(item=>item.id!==draft.id));setDraft(empty);showToast("규제 기준을 삭제했습니다.");}}/></form></div></ReferencePanel>;
}
